...
pub struct NeighCSR {
    pub pairs: Vec<(u16, i16)>,
    pub offsets: Vec<u32>,
}

impl NeighCSR {
    #[inline(always)]
    pub fn row(&self, i: usize) -> &[(u16, i16)] {
        let lo = unsafe { *self.offsets.get_unchecked(i) } as usize;
        let hi = unsafe { *self.offsets.get_unchecked(i + 1) } as usize;
        unsafe { self.pairs.get_unchecked(lo..hi) }
    }
}
...
pub fn build_sparse_neighbors_and_totals(ch: &Challenge) -> (NeighCSR, Vec<i64>) {
    let n = ch.num_items;
    let mut tmp: Vec<Vec<(u16, i16)>> = (0..n).map(|_| Vec::with_capacity(12)).collect();
    ...
    for i in 0..n {
        ...
        for j in 0..i {
            ...
            if val != 0 {
                let v16 = val as i16;
                tmp[i].push((j as u16, v16));
                tmp[j].push((i as u16, v16));
                let vv = val as i64;
                totals[i] += vv;
                totals[j] += vv;
            }
    ...
    let mut pairs: Vec<(u16, i16)> = Vec::with_capacity(total_pairs);
    let mut cursor: u32 = 0;
    offsets.push(0);
    for row in tmp.into_iter() {
        cursor = cursor.saturating_add(row.len() as u32);
        pairs.extend(row);
        offsets.push(cursor);
    }

    (NeighCSR { pairs, offsets }, totals)
}
