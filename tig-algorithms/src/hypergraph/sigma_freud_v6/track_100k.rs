...
pub fn solve(
    ...
    let compute_moves_kernel = module.load_function("compute_refinement_moves_optimized_100k")?;
    let execute_moves_kernel = module.load_function("execute_refinement_moves_100k")?;
    let balance_kernel = module.load_function("balance_final_100k")?;
    let reset_counters_kernel = module.load_function("reset_counters_100k")?;
    let compute_connectivity_kernel = module.load_function("compute_connectivity_100k")?;
    let perturb_kernel = module.load_function("perturb_solution_100k")?;
    let perturb_guided_kernel = module.load_function("perturb_guided_100k")?;
    ...
    let swap_exhaustive_limit = 32usize;
    let cycle_scan_limit = 8usize;
    ...
    macro_rules! do_swap_phase {
        ($d_partition:expr, $d_nodes_in_part:expr,
         ...
         $max_rounds:expr, $ngt:expr, $exh_lim:expr, $do_cycles:expr) => {{
            ...
            for _swap_round in 0..$max_rounds {
                ...
                for a in 0..np {
                    for b in 0..np {
                        if !$part_to_part[a * np + b].is_empty() {
                            $part_to_part[a * np + b].sort_unstable_by(|x, y| y.1.cmp(&x.1));
                        }
                    }
                }
                            ...
                            for c in 0..np {
                                    ...
                                    && best_ic < ca_top
                                {
                                    ...
                                    if $partition_mut_swap[node_a] as usize == a
                                        ...
                                        && $partition_mut_swap[node_c] as usize == c
                                    {
                                        $partition_mut_swap[node_a] = b as i32;
                                        $partition_mut_swap[node_b] = c as i32;
                                        $partition_mut_swap[node_c] = a as i32;
                                        swap_count += 1;
                                    }
                                }
                            }
