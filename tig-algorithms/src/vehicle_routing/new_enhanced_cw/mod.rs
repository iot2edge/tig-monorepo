...
mod dead_code {
    ...
    pub fn solve_sub_instance(challenge: &SubInstance) -> anyhow::Result<Option<SubSolution>> {
        ...
        for _ in 0..4 {
            ...
            for i in 0..NUM_ITERATIONS {
                let (neighbor_params, modified_indices) =
    ...
    impl Savings {
        ...
        pub fn recompute_savings(&mut self, params: &[u32], modified_indices: &[usize]) {
            ...
            let mut reduced_savings =
                Vec::with_capacity(modified_indices.len() * modified_indices.len());
    ...
    #[inline]
    fn create_solution(challenge: &SubInstance, savings_list: &[(u32, u8, u8)]) -> SubSolution {
        let num_nodes = challenge.num_nodes;
    ...
    unsafe fn two_opt_best_unsafe(route: &mut Vec<usize>, distance_matrix: &Vec<Vec<i32>>) -> bool {
        ...
        for i in 1..(n - 2) {
            ...
            for j in (i + 1)..(n - 1) {
                ...
                let gain = distance_matrix[ri_m1][ri] + distance_matrix[rj][rj_p1]
