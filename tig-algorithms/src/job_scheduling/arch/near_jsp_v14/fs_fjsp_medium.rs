...
fn find_candidate_insert_positions_fm(ds: &DisjSchedule, starts: &[u32], node: usize, new_machine: usize, new_pt: u32, job_pred: &[usize]) -> Vec<usize> {
    let seq=&ds.machine_seq[new_machine]; if seq.is_empty(){return vec![0];}
    let jp=job_pred[node]; let job_pred_end=if jp!=NONE_USIZE{starts[jp].saturating_add(ds.node_pt[jp])}else{0};
    let mut candidates: Vec<(usize,u32)>=Vec::with_capacity(seq.len()+1);
    for pos in 0..=seq.len(){
        let machine_pred_end=if pos>0{let pred=seq[pos-1];starts[pred].saturating_add(ds.node_pt[pred])}else{0};
        let my_start=job_pred_end.max(machine_pred_end); candidates.push((pos,my_start.saturating_add(new_pt)));
    }
    candidates.sort_by_key(|&(_,end)|end);
    candidates.into_iter().take(5).map(|(pos,_)|pos).collect()
}
...
pub fn solve(
    ...
    for r in 0..num_restarts {
        ...
        if mk<best_makespan{best_makespan=mk;best_solution=Some(sol.clone());save_solution(&sol)?;stuck=0;if learn_updates_left>0&&!pre.chaotic_like{learned_jb=Some(job_bias_from_solution(pre,&sol)?);learned_mp=Some(machine_penalty_from_solution(pre,&sol,challenge.num_machines)?);if route_w_base>0.0{learned_rp=Some(route_pref_from_solution_lite(pre,&sol,challenge)?);}learn_updates_left-=1;}}else{stuck=stuck.saturating_add(1);}
        push_top_solutions(&mut top_solutions,&sol,mk,15);
    }
    ...
    let ts_starts=top_solutions.len().min(15); let ts_iters=effort.fjsp_medium_iters;
    ...
    if let Some(ref sol)=best_solution{if let Some((improved_sol,improved_mk))=greedy_reassign_pass(pre,challenge,sol)?{if improved_mk<best_makespan{best_solution=Some(improved_sol.clone());save_solution(&improved_sol)?;}}}
