...
pub struct LocalOps<'a> {
    ...
    gran_keep: usize,
    cap_swap_flat: Vec<usize>,
    cap_swap_lens: Vec<usize>,
    gran2_max: usize,
    pub params: Config,
    pub cost: i64,
    ...
    pub nb_moves: usize,
    buf_left: Vec<RouteEval>,
    buf_right: Vec<RouteEval>,
    buf_left2: Vec<RouteEval>,
    buf_right2: Vec<RouteEval>,
    c1_order: Vec<usize>,
    singletons: Vec<RouteEval>,
}
...
impl<'a> LocalOps<'a> {
    pub fn new(data: &'a Instance, params: Config) -> Self {
        ...
        let gran_keep = min(params.granularity as usize, cap);
        let mut neighbors_flat = vec![0usize; n * gran_keep];
        ...
        for i in 1..n {
            ...
            let base = i * gran_keep;
            for (k, &(_, j)) in prox[..gran_keep].iter().enumerate() {
                neighbors_flat[base + k] = j;
            }
        }

        let gran2_max = params.granularity2 as usize;
        let mut cap_swap_flat = vec![0usize; n * gran2_max];
        let mut cap_swap_lens = vec![0usize; n];
        ...
        for i in 1..n {
            ...
            let m = prox.len().min(gran2_max);
            cap_swap_lens[i] = m;
            let base = i * gran2_max;
            for (k, &(_, j)) in prox[..m].iter().enumerate() {
                cap_swap_flat[base + k] = j;
            }
        }

        let fleet = data.nb_vehicles;
        let max_route_len = n + 2;
        let singletons: Vec<RouteEval> = (0..n).map(|i| RouteEval::singleton(data, i)).collect();
        Self {
            data,
            neighbors_flat,
            gran_keep,
            ...
            when_last_modified: vec![0; fleet],
            when_last_tested: vec![0; n],
            nb_moves: 0,
            buf_left: vec![RouteEval::default(); max_route_len],
            buf_right: vec![RouteEval::default(); max_route_len],
            buf_left2: vec![RouteEval::default(); max_route_len],
            buf_right2: vec![RouteEval::default(); max_route_len],
            c1_order: (1..n).collect(),
            singletons,
        }
    }
    ...
    fn load_from_routes(&mut self, routes: &Vec<Vec<usize>>) {
        ...
        let mut src: Vec<Vec<usize>> = Vec::with_capacity(fleet);
        ...
        self.routes = src.iter().map(|r| Route::new(r)).collect();
        for v in self.node_route.iter_mut() {
            *v = 0;
        }
        for v in self.node_pos.iter_mut() {
            *v = 0;
        }
        self.empty_routes.clear();

        let nr = self.routes.len();
        if self.when_last_modified.len() < nr {
            self.when_last_modified.resize(nr, 0);
        }
        for v in self.when_last_modified[..nr].iter_mut() {
            *v = 0;
        }
        for v in self.when_last_tested.iter_mut() {
            *v = 0;
        }
        self.nb_moves = 1;
    ...
    fn update_route(&mut self, rid: usize) {
        let data = self.data;
        let sings = &self.singletons;
        let r = &mut self.routes[rid];
        let len = r.nodes.len();

        let mut acc_fwd = sings[r.nodes[0].id];
        r.nodes[0].seq0_i = acc_fwd;
        for pos in 1..len {
            acc_fwd = RouteEval::join2(data, &acc_fwd, &sings[r.nodes[pos].id]);
            r.nodes[pos].seq0_i = acc_fwd;
        }

        let mut acc_bwd = sings[r.nodes[len - 1].id];
        r.nodes[len - 1].seqi_n = acc_bwd;
        for pos in (0..len - 1).rev() {
            acc_bwd = RouteEval::join2(data, &sings[r.nodes[pos].id], &acc_bwd);
            r.nodes[pos].seqi_n = acc_bwd;
        }

        for pos in 0..len {
            r.nodes[pos].seq1 = sings[r.nodes[pos].id];
            if pos + 1 < len {
                let s1 = r.nodes[pos].seq1;
                let s2 = sings[r.nodes[pos + 1].id];
                r.nodes[pos].seq12 = RouteEval::join2(data, &s1, &s2);
    ...
    pub fn run_intra_route_relocate(&mut self, r1: usize, pos1: usize) -> bool {
        ...
        let left_excl = &mut self.buf_left;
        ...
        let right_excl = &mut self.buf_right;
    ...
    pub fn run_intra_route_oropt(&mut self, r1: usize, pos1: usize, l: usize) -> bool {
        ...
        let applied = {
            ...
            let block_seq = if l == 2 {
                ...
                // seq123 computed on-the-fly
                RouteEval::join2(
                    self.data,
                    &route.nodes[pos1].seq12,
                    &self.singletons[route.nodes[pos1 + 2].id],
                )
            };
    ...
    pub fn run_inter_route(&mut self, r1: usize, pos1: usize, r2: usize, pos2: usize) -> bool {
        ...
        if x.id != 0 {
            ...
            if x_next.id != 0 && self.params.allow_swap3 {
                // seq123 computed on-the-fly
                let u_seq123 = RouteEval::join2(data, &u.seq12, &self.singletons[x_next.id]);
                ...
                if v.id != 0 {
                    ...
                    if y.id != 0 {
                        ...
                        if y_next.id != 0 {
                            let v_seq123 =
                                RouteEval::join2(data, &v.seq12, &self.singletons[y_next.id]);
    ...
    pub fn run_swapstar(&mut self, r1: usize, pos1: usize, r2: usize, pos2: usize) -> bool {
        ...
        let left_excl1 = &mut self.buf_left;
        let right_excl1 = &mut self.buf_right;
        {
        ...
        let left_excl2 = &mut self.buf_left2;
        let right_excl2 = &mut self.buf_right2;
        {
    ...
    pub fn runls(
        ...
        while improved {
            improved = false;
            loop_id += 1;
            self.c1_order.shuffle(rng);
            for ci in 0..self.c1_order.len() {
                let c1 = self.c1_order[ci];
                ...
                let gran = self.gran_keep;
                let nb_base = c1 * gran;
                let start = rng.gen_range(0..gran);
                let mut nb_idx = start;
                for _ in 0..gran {
                    let c2 = unsafe { *self.neighbors_flat.get_unchecked(nb_base + nb_idx) };
                    nb_idx += 1;
                    if nb_idx >= gran {
                        nb_idx = 0;
                    }
                ...
                let r1 = self.node_route[c1];
                let pos1 = self.node_pos[c1];
                let swap_len = self.cap_swap_lens[c1];
                if swap_len > 0 {
                    let cs_base = c1 * self.gran2_max;
                    let start_s = rng.gen_range(0..swap_len);
                    let mut cs_idx = start_s;
                    for _ in 0..swap_len {
                        let c2 = unsafe { *self.cap_swap_flat.get_unchecked(cs_base + cs_idx) };
                        cs_idx += 1;
                        if cs_idx >= swap_len {
                            cs_idx = 0;
                        }
