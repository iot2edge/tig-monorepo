...
pub fn tabu_search_phase(
    ...
    let kick_threshold = (max_no_improve * 2 / 3).max(40);
    ...
    for iter in 0..max_iterations {
        ...
        match chosen {
            Some((m, pos, _est)) => {
                ...
                if succ_b_node != NONE_USIZE {
                    machine_pred_node[succ_b_node] = node_a;
                }
            }
            None => break,
        }
...
fn estimate_insert_mk_disj(
    ...
    let mp = if new_pos_adj > 0 {
        at(new_pos_adj - 1)
    } else {
        NONE_USIZE
    };
    let ms = if new_pos_adj < cleaned_len {
        at(new_pos_adj)
    } else {
        NONE_USIZE
    };
    let old_mp = if old_pos > 0 {
        seq[old_pos - 1]
    } else {
        NONE_USIZE
    };
    let old_ms = if old_pos + 1 < slen {
        seq[old_pos + 1]
    } else {
        NONE_USIZE
    };
    let jp_u = job_pred[u];
    let js_u = ds.job_succ[u];
    let r_j = if jp_u != NONE_USIZE {
        heads[jp_u].saturating_add(ds.node_pt[jp_u])
    } else {
        0
    };
    let r_m = if mp != NONE_USIZE {
        heads[mp].saturating_add(ds.node_pt[mp])
    } else {
        0
    };
    let new_r = r_j.max(r_m);
    let q_j = if js_u != NONE_USIZE {
        ds.node_pt[js_u].saturating_add(tails[js_u])
    } else {
        0
    };
    let q_m = if ms != NONE_USIZE {
        ds.node_pt[ms].saturating_add(tails[ms])
    } else {
        0
    };
    ...
    let path_gap = if old_ms != NONE_USIZE {
        ...
        let r_j2 = if jp2 != NONE_USIZE {
            heads[jp2].saturating_add(ds.node_pt[jp2])
        } else {
            0
        };
        let r_m2 = if old_mp != NONE_USIZE {
            heads[old_mp].saturating_add(ds.node_pt[old_mp])
        } else {
            0
        };
        r_j2.max(r_m2)
    ...
    let path_ms = if ms != NONE_USIZE {
        let jp3 = job_pred[ms];
        let r_j3 = if jp3 != NONE_USIZE {
            heads[jp3].saturating_add(ds.node_pt[jp3])
        } else {
            0
        };
        let r_u_end = new_r.saturating_add(pt_u);
        r_j3.max(r_u_end)
            .saturating_add(ds.node_pt[ms])
            .saturating_add(tails[ms])
    } else {
        0
    };
...
fn tabu_n7_phase(
    ...
    let mut pseed: u64 = 0x9E3779B9_u64
        .wrapping_mul(initial_mk as u64)
    ...
    'outer: for iter in 0..max_iterations {
        ...
        if use_n7 {
            n7_eval_count += 1;
            ins_tabu.retain(|_, &mut exp| exp > iter);
        }
        let mut best_swap: Option<(usize, usize)> = None;
        let mut best_swap_mk = u32::MAX;
        let mut fb_swap: Option<(usize, usize)> = None;
        let mut fb_swap_mk = u32::MAX;
        ...
        for m in 0..ds.num_machines {
            ...
            while i < slen {
                ...
                if bs + 1 < slen {
                    ...
                    let is_tabu = tabu.get(&key).map_or(false, |&e| iter < e);
                    let asp = est < best_mk;
                    if (!is_tabu || asp) && est < best_swap_mk {
                        best_swap_mk = est;
                        best_swap = Some((m, bs));
                    }
                    if est < fb_swap_mk {
                ...
                if use_n7 && be > bs {
                    ...
                    if !ins_tabu.contains_key(&u_a) {
                        let est_a = estimate_insert_mk_disj(
                            u_a,
                            m,
                            bs,
                            be,
                        ...
                        if est_a < best_mk && est_a < best_ins_mk {
                            best_ins_mk = est_a;
                            best_ins = Some((m, bs, be));
                        }
                    }
                    let u_b = seq2[be];
                    if !ins_tabu.contains_key(&u_b) {
                        let est_b = estimate_insert_mk_disj(
                            u_b,
                            m,
                            be,
                            bs,
                        ...
                        if est_b < best_mk && est_b < best_ins_mk {
                            best_ins_mk = est_b;
                            best_ins = Some((m, be, bs));
                        }
                    }
                }
                i = be + 1;
        ...
        if best_swap_mk <= best_ins_mk {
            if let Some((m, pos)) = best_swap {
                let na = ds.machine_seq[m][pos];
                let nb = ds.machine_seq[m][pos + 1];
                let pred_a = machine_pred_node[na];
                let succ_b = buf.machine_succ[nb];
                ds.machine_seq[m].swap(pos, pos + 1);
                let key = (na.min(nb), na.max(nb));
                tabu.insert(key, iter + tenure.saturating_sub(tenure_delta) + off);
                machine_pred_node[nb] = pred_a;
                machine_pred_node[na] = nb;
                if succ_b != NONE_USIZE {
                    machine_pred_node[succ_b] = na;
                }
    ...
    let Some((mk_final, _)) = eval_disj(&best_ds, &mut buf) else {
        return Ok(None);
    };
...
fn path_relinking_phase(
    ...
    if pool.len() < 2 || steps_per_pair == 0 {
        return Ok(None);
    }
    let n = pre.total_ops;
    ...
    // Static job_pred_node (same for all pairs)
    {
        ...
        for j in 0..proto.num_jobs {
            let base_j = proto.job_offsets[j];
            let end_j = proto.job_offsets[j + 1];
            for k in (base_j + 1)..end_j {
                job_pred_node[k] = k - 1;
            }
        }
    }
    ...
    for target_idx in 1..=num_pairs {
        ...
        for _step in 0..steps {
            let Some((cur_mk, _)) = eval_disj(&current, &mut buf) else {
                break;
            };
            if cur_mk < best_mk {
                best_mk = cur_mk;
                if let Ok(s) = disj_to_solution(pre, &current, &buf.start) {
                    best_found = Some(s);
                }
            }
            ...
            machine_pred_node.fill(NONE_USIZE);
            for seq in &current.machine_seq {
                for i in 1..seq.len() {
                    machine_pred_node[seq[i]] = seq[i - 1];
                }
            }
            ...
            back_deg.fill(0);
            for i in 0..n {
                if current.job_succ[i] != NONE_USIZE {
                    back_deg[i] += 1;
                }
                if buf.machine_succ[i] != NONE_USIZE {
                    back_deg[i] += 1;
            ...
            for m in 0..current.num_machines {
                if current.machine_seq[m].len() <= 1 {
                    continue;
                }
            ...
            if !any_inversion {
                break;
            } // path complete: current reached target
            ...
            let (m, pos) = best_move.unwrap_or_else(|| {
                for m in 0..current.num_machines {
                    for i in 0..(current.machine_seq[m].len().saturating_sub(1)) {
                        let a = current.machine_seq[m][i];
                        let b = current.machine_seq[m][i + 1];
                        if a < n && b < n && target_pos[a] > target_pos[b] {
                            return (m, i);
                        }
                    }
                }
...
pub fn solve(
    ...
    push_diverse(
        ...
        greedy_sol.clone(),
        greedy_mk,
        machine_signature(&greedy_sol, pre),
        25,
    );
    ...
    let ts_tenure =
        ((pre.total_ops as f64).sqrt() as usize * (100 + (pre.load_cv * 60.0) as usize) / 100)
            .clamp(8, 24);
    ...
    if warm_start {
        ...
        if let Ok(Some((rep, mk))) =
            tabu_search_phase(pre, challenge, &seed_sol, REPAIR_ITERS, ts_tenure)
        {
            if mk < best_makespan {
                best_makespan = mk;
                best_solution = Some(rep.clone());
                save_solution(&rep)?;
            }
        ...
        {
            let ts_pool: Vec<Solution> = top_solutions
                .iter()
                .take(ts_starts)
                .map(|(s, _)| s.clone())
                .collect();
        ...
        {
            let bn_pool: Vec<Solution> = top_solutions
                .iter()
                .take(8)
                .map(|(s, _)| s.clone())
                .collect();
            for base_sol in &bn_pool {
                let mut ds = match build_disj_from_solution(pre, challenge, base_sol) {
                    Ok(d) => d,
                    Err(_) => continue,
                };
                let mut buf = EvalBuf::new(ds.n);
                    ...
                    if m < ds.machine_seq.len() {
                        for &nd in &ds.machine_seq[m] {
                            if nd < ds.node_pt.len() {
                                machine_total_pt[m] =
                                    machine_total_pt[m].saturating_add(ds.node_pt[nd] as u64);
                            }
                        }
                    }
                }
                let mut m_rank: Vec<usize> = (0..challenge.num_machines)
                    .filter(|&m| m < ds.machine_seq.len() && ds.machine_seq[m].len() > 1)
                    .collect();
                ...
                if any_improved {
                    if let Some((mk_bn, _)) = eval_disj(&ds, &mut buf) {
                        if let Ok(sol_bn) = disj_to_solution(pre, &ds, &buf.start) {
                            if mk_bn < best_makespan {
                                best_makespan = mk_bn;
                                best_solution = Some(sol_bn.clone());
                                save_solution(&sol_bn)?;
                            }
        ...
        {
            let ils_pool: Vec<Solution> = top_solutions
                .iter()
                .take(6)
                .map(|(s, _)| s.clone())
                .collect();
            for base_sol in &ils_pool {
                if let Ok(Some((ls_sol, ls_mk))) =
                    critical_block_move_local_search_ex(pre, challenge, base_sol, 5, 400, 120)
                {
                    ...
                    if let Some((sol3, mk3)) =
                        tabu_search_phase(pre, challenge, &ls_sol, ts_iters / 2, ts_tenure)?
                    {
...

    {
        ...
        let mut machine_rank: Vec<(usize, f64)> = (0..num_machines)
            .map(|m| {
                let scar = if m < pre.machine_scarcity.len() {
                    pre.machine_scarcity[m]
                } else {
                    1.0
                };
                (m, scar)
        ...
        for i in 0..num_bn {
            is_bottleneck[machine_rank[i].0] = true;
        }
        ...
        if !diverse_pool.is_empty() {
            ...
            let mut sorted_diverse: Vec<(usize, usize)> = diverse_pool
                .iter()
                .enumerate()
        ...
        for gen in 0..num_generations {
            if phase6_start.elapsed().as_millis() >= phase6_budget_ms {
                break;
            }
            ...
            let ds_a = match build_disj_from_solution(pre, challenge, &sol_a) {
                Ok(d) => d,
                Err(_) => {
                    gen_no_improve += 1;
                    continue;
            ...
            let ds_b = match build_disj_from_solution(pre, challenge, &sol_b) {
                Ok(d) => d,
                Err(_) => {
                    gen_no_improve += 1;
                    continue;
            ...
            let (better_ds, worse_ds) = if mk_a <= mk_b {
                (&ds_a, &ds_b)
            } else {
                (&ds_b, &ds_a)
            };
            ...
            for m in 0..num_machines {
                ...
                if m >= worse_ds.machine_seq.len() || m >= child_ds.machine_seq.len() {
                    continue;
                }
            ...
            if use_mutation {
                let non_bn_machines: Vec<usize> = (0..num_machines)
                    .filter(|&m| !is_bottleneck[m] && child_ds.machine_seq[m].len() > 1)
                    .collect();
                ...
                let bn_machines: Vec<usize> = (0..num_machines)
                    .filter(|&m| is_bottleneck[m] && child_ds.machine_seq[m].len() > 1)
                    .collect();
            ...
            let (mut ls_sol, mut ls_mk) = match critical_block_move_local_search_ex(
                pre, challenge, &child_sol, 4, 250, 100,
            ) {
                Ok(Some((s, mk))) => (s, mk),
                            ...
                            if let (Some(ps), Some(pe)) = (
                                ...
                                if ps != pe {
                                    ...
                                    if eval_disj(&ds, &mut buf).is_some() {
                                        if let Ok(perturbed_sol) =
                                            disj_to_solution(pre, &ds, &buf.start)
                                        {
                                            if let Ok(Some((re_sol, re_mk))) =
            ...
            if cur_pop >= pop_cap {
                let worst_idx = mem_pop
                    .iter()
                    .enumerate()
                    .max_by_key(|(_, (_, mk))| *mk)
                    .map(|(i, _)| i)
                    .unwrap_or(cur_pop - 1);
        ...
        for base_sol in &mem_best {
            if let Some((ts_sol, ts_mk)) =
                tabu_search_phase(pre, challenge, base_sol, ts_iters / 3, ts_tenure)?
            {
    ...
    // exact reinsertion evaluation, repaired by CBMLS, intensified by tabu(20k).
    {
        ...
        while ils7_start.elapsed().as_millis() < ils7_budget_ms && no_global_improve < 25 {
            ...
            let (repaired_sol, _) = match critical_block_move_local_search_ex(
                pre, challenge, &perturbed, 4, 250, 100,
            ) {
            ...
            if let Ok(mut ds) = build_disj_from_solution(pre, challenge, curr_best) {
                ...
                if let Some((_mk, terminal)) = eval_disj(&ds, &mut buf) {
                    ...
                    while u != NONE_USIZE {
                        ...
                        let p = pos[u];
                        if p > 0 {
                            swap_cands.push((m, p - 1));
                        }
                        if p + 1 < ds.machine_seq[m].len() {
                            swap_cands.push((m, p));
                        }
                        u = buf.best_pred[u];
                    }
    ...
    if let Some(ref final_best) = best_solution.clone() {
        if let Some((sol4, mk4)) =
            tabu_search_phase(pre, challenge, final_best, ts_iters, ts_tenure)?
        {
    ...
    if let Ok(Some((pr_sol, pr_mk))) =
        path_relinking_phase(pre, challenge, &top_solutions, best_makespan, 4_000)
    {
        ...
        if let Ok(mut ds) = build_disj_from_solution(pre, challenge, best_after_pr) {
            ...
            if let Some((_mk, terminal)) = eval_disj(&ds, &mut buf) {
                ...
                if let Ok(perturbed_sol) = disj_to_solution(pre, &ds, &buf.start) {
                    ...
                    if let Ok(Some((n7_sol, n7_mk))) =
                        tabu_n7_phase(pre, challenge, &perturbed_sol, 15_000, n7_tenure)
                    {
    ...
    // Fuel-based (no wall-clock). num_sweeps = effort.job_shop_iters / 2000 ≈ 50 at default.
    {
        ...
        if num_sweeps > 0 {
            if let Some(ref current_best) = best_solution.clone() {
                if let Ok(Some((dils_sol, dils_mk))) =
                    dils_schrage_phase(pre, challenge, current_best, num_sweeps, &mut rng)
                {
