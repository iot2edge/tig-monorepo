...
pub struct LocalOps<'a> {
    ...
    pub is_intra_neighbor: Vec<bool>,
    pub params: Config,
    pub cost: i64,
...
impl<'a> LocalOps<'a> {
    pub fn new(data: &'a Instance, params: Config) -> Self {
        ...
        // FILO-style symmetric granular neighborhood for intra-route moves.
        // Marks (c1,c2) true if c2 is among c1's K closest (or vice-versa).
        // Depot (id 0) is treated as a neighbor of everyone so positions
        // adjacent to the depot are never skipped.
        let k_intra = min(params.granularity_intra as usize, cap);
        let mut is_intra_neighbor = vec![false; n * n];
        for i in 0..n {
            is_intra_neighbor[i] = true;
            is_intra_neighbor[i * n] = true;
        }
        for i in 1..n {
            let mut prox: Vec<(i32, usize)> = Vec::with_capacity(n.saturating_sub(2));
            for j in 1..n {
                if j == i {
                    continue;
                }
            ...
            prox.sort_by_key(|&(d, _)| d);
            let m = prox.len().min(k_intra);
            for &(_, j) in prox.iter().take(m) {
                is_intra_neighbor[i * n + j] = true;
                is_intra_neighbor[j * n + i] = true;
            }
        }
    ...
    fn update_route(&mut self, rid: usize) {
        ...
        r.nodes[0].seq1 = acc_fwd;
        r.nodes[0].seq0_i = acc_fwd;
        for pos in 1..len {
            let s1 = RouteEval::singleton(data, r.nodes[pos].id);
            r.nodes[pos].seq1 = s1;
    ...
    pub fn run_intra_route_relocate(&mut self, r1: usize, pos1: usize) -> bool {
        ...
        self.scratch_a.resize(len, RouteEval::default());
        self.scratch_b.resize(len, RouteEval::default());

        let mut acc_left = self.routes[r1].nodes[0].seq0_i;
        for p in 1..len {
            self.scratch_a[p] = acc_left;
            if p != pos1 {
                acc_left = RouteEval::join2(self.data, &acc_left, &self.routes[r1].nodes[p].seq1);
            }
        }

        let mut acc_right = self.routes[r1].nodes[len - 1].seq1;
        self.scratch_b[len - 1] = acc_right;
        for p in (1..len - 1).rev() {
            if p != pos1 {
                acc_right = RouteEval::join2(self.data, &self.routes[r1].nodes[p].seq1, &acc_right);
            }
            self.scratch_b[p] = acc_right;
        }
        ...
        let c1_id = self.routes[r1].nodes[pos1].id;
        let n = self.data.nb_nodes;
        ...
        for t in 1..len {
            ...
            let prev_id = self.routes[r1].nodes[t - 1].id;
            let next_id = self.routes[r1].nodes[t].id;
            if !self.is_intra_neighbor[c1_id * n + prev_id]
                && !self.is_intra_neighbor[c1_id * n + next_id]
            {
                continue;
            }
    ...
    pub fn run_intra_route_swap_right(&mut self, r1: usize, pos1: usize) -> bool {
        ...
        let c1_id = route.nodes[pos1].id;
        let n = self.data.nb_nodes;
        for pos2 in (pos1 + 2)..(len - 1) {
            let c2_id = route.nodes[pos2].id;
            if !self.is_intra_neighbor[c1_id * n + c2_id] {
    ...
    pub fn run_2opt(&mut self, r1: usize, pos1: usize) -> bool {
        ...
        let anchor_id = route.nodes[pos1 - 1].id;
        let n = self.data.nb_nodes;
        for pos2 in (pos1 + 1)..(len - 1) {
            let pos2_id = route.nodes[pos2].id;
            if self.is_intra_neighbor[anchor_id * n + pos2_id] {
                let new_cost = RouteEval::eval3(
                    self.data,
                    &self.params,
    ...
    pub fn run_intra_route_oropt(&mut self, r1: usize, pos1: usize, l: usize) -> bool {
        ...
        let applied = {
            ...
            let block_first_id = route.nodes[pos1].id;
            let block_last_id = route.nodes[pos1 + l - 1].id;
            let n = self.data.nb_nodes;
            if pos1 > 1 {
                let mut mid_seq = route.nodes[pos1 - 1].seq1;
                for t in (1..pos1).rev() {
                    let prev_id = route.nodes[t - 1].id;
                    let next_id = route.nodes[t].id;
                    if self.is_intra_neighbor[block_first_id * n + prev_id]
                        || self.is_intra_neighbor[block_last_id * n + next_id]
                    {
    ...
    pub fn run_swapstar(&mut self, r1: usize, pos1: usize, r2: usize, pos2: usize) -> bool {
        ...
        self.scratch_a.resize(route1_len, RouteEval::default());
        self.scratch_b.resize(route1_len, RouteEval::default());
        {
            ...
            self.scratch_b[route1_len - 1] = acc_right;
        ...
        self.scratch_c.resize(route2_len, RouteEval::default());
        self.scratch_d.resize(route2_len, RouteEval::default());
        {
            let r = &self.routes[r2];
            let mut acc_left = r.nodes[0].seq0_i;
            for p in 1..route2_len {
                self.scratch_c[p] = acc_left;
                if p != pos2 {
                    acc_left = RouteEval::join2(self.data, &acc_left, &r.nodes[p].seq1);
                }
            }
            let mut acc_right = r.nodes[route2_len - 1].seq1;
            self.scratch_d[route2_len - 1] = acc_right;
            for p in (1..route2_len - 1).rev() {
                if p != pos2 {
                    acc_right = RouteEval::join2(self.data, &r.nodes[p].seq1, &acc_right);
                }
                self.scratch_d[p] = acc_right;
            }
        }
        ...
        while improved {
            ...
            for &c1 in &c1_order {
                ...
                while changed {
                    ...
                    let neighs: Vec<usize> = self.neighbors_before[c1].clone();
                    for c2 in neighs {
                        ...
                        if self.when_last_modified[r1].max(self.when_last_modified[r2])
                            <= last_tested
                        {
                            continue;
                        }
                        ...
                        let r1n = self.node_route[c1];
                        let pos1n = self.node_pos[c1];
                        if pos1n == 1 && self.run_inter_route(r2, pos2, r1n, pos1n) {
                            improved = true;
                            changed = true;
                            break;
                        }
                        ...
                        let r1n = self.node_route[c1];
                        let pos1n = self.node_pos[c1];
                        if self.run_2optstar(r1n, pos1n, r2, pos2 + 1) {
                            improved = true;
                            changed = true;
                            break;
                        }
                ...
                if swap_len > 0 {
                    ...
                    for _ in 0..swap_len {
                        let c2 = self.neighbors_capacity_swap[c1][idx_s];
                        idx_s += 1;
                        if idx_s == swap_len {
                            idx_s = 0;
                        }
