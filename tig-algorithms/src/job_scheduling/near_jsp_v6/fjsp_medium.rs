...
fn score_candidate(
    ...
    let slack_reg_boost = 1.0 + 0.40 * reg_n * progress;
    ...
    let route_term = if route_w > 0.0 && op.flex >= 2 {
        route_w * route_gain * route_pref_bonus_fm(route_pref, product, op_idx, machine)
    } else {
        0.0
    };
    match rule {
        ...
        Rule::Regret => {
            ...
            let base_score = (reg_scale * 1.10 * reg_n)
                + (0.60 * rem_min_n)
                + (0.25 * scarcity_urg)
                + (scarce_w * pre.flex_factor) * flex_inv
                + next_term
                - (0.65 * end_n)
                - pop_pen
                + flow_term
        ...
        Rule::Adaptive => {
            ...
            let reg_scale = (1.0 + 0.40 * pre.bn_focus * (1.0 / pre.flex_avg.max(1.0)) * 2.5)
                * (1.0 + 0.30 * pre.load_cv);
...
fn construct_solution_job_centric(challenge: &Challenge, pre: &Pre) -> Result<(Solution, u32)> {
    let num_jobs = challenge.num_jobs;
    let num_machines = challenge.num_machines;
...
fn exhaustive_critical_reroute_pass(
    pre: &Pre,
    challenge: &Challenge,
    base_sol: &Solution,
...
fn greedy_reassign_pass(
    ...
    while passes < max_passes {
        ...
        let Some((cur_mk, _)) = eval_disj(&ds, &mut buf) else {
            break;
        };
        current_mk = cur_mk;
        ...
        let Some((node, best_m, best_pt, best_ins_pos)) = best_pass_move else {
            break;
        };
        ...
        for m in 0..ds.num_machines {
            ...
            for &(bstart, bend) in &blocks {
                ...
                for si in 0..num_swaps {
                    ...
                    let est_mk = estimate_swap_mk_fm(
                        node_u,
                        node_v,
                        &buf.start,
                        &tail,
        ...
        if iter % reassign_freq == 0 {
            for node in 0..n {
                ...
                for &(new_m, new_pt) in &op_info.machines {
                    ...
                    let positions = find_candidate_insert_positions_fm(
                        &ds,
                        &buf.start,
                        node,
                        new_m,
                    ...
                    for insert_pos in positions {
                        let est_mk = estimate_reassign_mk_fm(
                            &ds,
                            &buf.start,
                            &tail,
                            node,
...
pub fn solve(
    ...
    let route_w_base: f64 = if pre.chaotic_like {
        ...
        (0.050 + 0.12 * pre.high_flex + 0.10 * pre.jobshopness + (0.08 / pre.flex_avg.max(1.0)))
            .clamp(0.04, 0.28)
    };
    ...
    let mut rule_best: Vec<u32> = vec![u32::MAX; 10];
    ...
    let base = best_solution
        .as_ref()
        .ok_or_else(|| anyhow!("No initial solution found"))?;
    ...
    if let Some(ref sol) = best_solution.clone() {
        if let Ok(Some((ecr_sol, ecr_mk))) = exhaustive_critical_reroute_pass(pre, challenge, sol) {
            if ecr_mk < best_makespan {
                best_makespan = ecr_mk;
                best_solution = Some(ecr_sol.clone());
                save_solution(&ecr_sol)?;
            }
        }
    }
    ...
    for ci in 0..cb_top_n {
        ...
        if let Ok(Some((cb_sol, cb_mk))) = critical_block_move_local_search_ex(
            pre,
            challenge,
            base_sol,
            cb_passes,
    ...
    if top_solutions.len() >= 4 {
        if let Ok(Some((ccs_sol, ccs_mk))) = consensus_critical_skeleton_pass(
            pre,
            challenge,
            &top_solutions,
            cb_passes + 1,
            ...
            if ccs_mk < best_makespan {
                best_makespan = ccs_mk;
                best_solution = Some(ccs_sol.clone());
                save_solution(&ccs_sol)?;
            }
            push_top_solutions(&mut top_solutions, &ccs_sol, ccs_mk, 20);
        }
    }

    let ils_rounds = if effort.fjsp_medium_iters > 300 {
        30
    } else {
        20
    };
    ...
    for ils_r in 0..ils_rounds {
        if ils_no_improve >= ils_max_no_improve {
            break;
        }
        let Some(ref base) = ils_best_sol.clone() else {
            break;
        };
        ...
        let n = ds.n;
        let mut perturb_seed: u64 = (ils_r as u64)
        ...
        if strategy == 0 {
            ...
            while perturbed < k_perturb && attempts < crit_nodes.len() * 4 {
                attempts += 1;
                perturb_seed ^= perturb_seed.wrapping_shl(13);
                perturb_seed ^= perturb_seed.wrapping_shr(7);
                perturb_seed ^= perturb_seed.wrapping_shl(17);
                ...
                for (ki, &nd) in ds.machine_seq[new_m].iter().enumerate() {
                    if buf.start[nd] >= cur_start {
                        ins_pos = ki;
                        break;
                    }
            ...
            for node in 0..n {
                ...
                machine_loads[m] = machine_loads[m].saturating_add(ds.node_pt[node]);
            }
            let worst_m = machine_loads
                .iter()
                .enumerate()
                .max_by_key(|&(_, &v)| v)
                .map(|(i, _)| i)
                .unwrap_or(0);
            if ds.machine_seq[worst_m].is_empty() {
                continue;
            }
            let mut perturbed = 0;
            let mut attempts = 0;
            while perturbed < k_perturb && attempts < ds.machine_seq[worst_m].len() * 4 {
                ...
                let mut best_alt_m = worst_m;
                let mut best_alt_pt = ds.node_pt[node];
                for &(am, apt) in &op_info.machines {
                    if am != worst_m && apt < best_alt_pt {
                        best_alt_pt = apt;
                        best_alt_m = am;
                    }
                }
                if best_alt_m == worst_m {
                    continue;
                }
                let old_pos = match ds.machine_seq[worst_m].iter().position(|&x| x == node) {
                    Some(p) => p,
                    None => continue,
                };
                ds.machine_seq[worst_m].remove(old_pos);
                ds.node_machine[node] = best_alt_m;
                ds.node_pt[node] = best_alt_pt;
                let cur_start = buf.start[node];
                let mut ins_pos = ds.machine_seq[best_alt_m].len();
                for (ki, &nd) in ds.machine_seq[best_alt_m].iter().enumerate() {
                    if buf.start[nd] >= cur_start {
                        ins_pos = ki;
                        break;
                    }
            ...
            while u != NONE_USIZE {
                ...
                if !crit_machines.contains(&m) {
                    crit_machines.push(m);
                }
                u = buf.best_pred[u];
            }
            ...
            for _ in 0..(k_swaps * 4) {
                if swapped >= k_swaps || crit_machines.is_empty() {
                    break;
                }
                perturb_seed ^= perturb_seed.wrapping_shl(13);
                perturb_seed ^= perturb_seed.wrapping_shr(7);
        ...
        let after_reassign = match greedy_reassign_pass(pre, challenge, &perturbed_sol)? {
            Some((s, mk)) => (s, mk),
            None => {
                if let Some((pmk, _)) = eval_disj(&ds, &mut buf) {
                    (perturbed_sol.clone(), pmk)
                } else {
                    ils_no_improve += 1;
                    continue;
                }
        ...
        let ls_result = critical_block_move_local_search_ex(
            pre,
            challenge,
            &after_reassign.0,
            cb_passes,
        ...
        op_weights[strategy] =
            op_weights[strategy] * (1.0 - LEARNING_RATE) + reward * LEARNING_RATE;

        if candidate_mk < best_makespan {
            best_makespan = candidate_mk;
            best_solution = Some(candidate_sol.clone());
            save_solution(&candidate_sol)?;
        }

        if candidate_mk < ils_best_mk {
            ils_best_mk = candidate_mk;
            ils_best_sol = Some(candidate_sol);
            ils_no_improve = 0;
        } else {
            ils_no_improve += 1;
        }
    ...
    {
        let alns_rounds = if effort.fjsp_medium_iters > 300 {
            50
        } else {
            35
        };
        ...
        let cooling = if alns_rounds > 1 {
            (t_final / t_init.max(1.0)).powf(1.0 / (alns_rounds as f64))
        } else {
            0.95
        };
        let mut temperature = t_init;
        let mut alns_seed: u64 = (challenge.seed[0] as u64)
            .wrapping_mul(0xB7E151628AED2A6Bu64)
            .wrapping_add(best_makespan as u64)
            .wrapping_add(0x9E3779B97F4A7C15u64);

        for alns_r in 0..alns_rounds {
            if alns_no_improve >= alns_max_no_improve {
                break;
            }
            let Some(ref base) = alns_sa_sol.clone() else {
                break;
            };
            let mut ds = match build_disj_from_solution(pre, challenge, base) {
                Ok(d) => d,
                Err(_) => {
                    alns_no_improve += 1;
                    temperature *= cooling;
            ...
            let mut buf = EvalBuf::new(ds.n);
            let Some((_cur_mk, mk_node)) = eval_disj(&ds, &mut buf) else {
                alns_no_improve += 1;
                temperature *= cooling;
                continue;
            };
            ...
            alns_seed ^= alns_seed.wrapping_shl(13);
            alns_seed ^= alns_seed.wrapping_shr(7);
            alns_seed ^= alns_seed.wrapping_shl(17);
            ...
            while !to_ins.is_empty() && rep_iter < max_repair {
                ...
                for (ti, &nd) in to_ins.iter().enumerate() {
                        ...
                        for pos in 0..=seq.len() {
                            ...
                            let succ_pen = if pos < seq.len() {
                                ...
                                if !removed_set[succ] {
                                    ...
                                    if new_succ_st > buf.start[succ] {
                                        (new_succ_st - buf.start[succ]) / 2
                                    } else {
                                        0
                                    }
                                } else {
                    ...
                    if node_bm == NONE_USIZE {
                        continue;
                    }
                    found_any = true;
                    let regret = if node_second < u32::MAX {
                        (node_second - node_best) as f64
                    } else {
                        pre.avg_op_min * 3.0
                    };
                ...
                if !found_any || best_ins_m == NONE_USIZE {
                    for ti in 0..to_ins.len() {
                        ...
                        if let Some(&(m, pt)) = op_info.machines.first() {
                            ...
                            ds.node_machine[nd] = m;
                            ds.node_pt[nd] = pt;
                            removed_set[nd] = false;
                            to_ins.remove(ti);
                            break;
                        }
                    }
                ...
                ds.node_machine[nd] = best_ins_m;
                ds.node_pt[nd] = best_ins_pt;
                removed_set[nd] = false;
                to_ins.remove(best_ni);
                let _ = eval_disj(&ds, &mut buf);
            }
            for &nd in &to_ins {
                let job = ds.node_job[nd];
            ...
            let Some((repaired_mk, _)) = eval_disj(&ds, &mut buf) else {
                alns_no_improve += 1;
                temperature *= cooling;
                continue;
            };
            let repaired_sol = match disj_to_solution(pre, &ds, &buf.start) {
                Ok(s) => s,
                Err(_) => {
                    alns_no_improve += 1;
                    temperature *= cooling;
            ...
            let (alns_cand_sol, alns_cand_mk) = if let Ok(Some((ls_sol, ls_mk))) =
                critical_block_move_local_search_ex(
                    pre,
                    challenge,
                    &after_gr.0,
                    cb_passes,
    ...
    if top_solutions.len() >= 5 {
        let vote_result2 = crossover_majority_vote(
            pre,
            challenge,
            &top_solutions,
            cb_passes + 1,
    ...
    if let Some(ref sol) = best_solution.clone() {
        if let Some((improved_sol, improved_mk)) = greedy_reassign_pass(pre, challenge, sol)? {
            if improved_mk < best_makespan {
                best_makespan = improved_mk;
                save_solution(&improved_sol)?;
                best_solution = Some(improved_sol);
            }
        }
    }
    ...
    {
        let final_ils_rounds = if effort.fjsp_medium_iters > 300 {
            12
        } else {
            8
        };
        ...
        let mut fpseed: u64 = (challenge.seed[0] as u64)
            .wrapping_mul(0xDEADC0DEu64)
            .wrapping_add(best_makespan as u64)
            .wrapping_add(0xFEEDFACEu64);

        for fir in 0..final_ils_rounds {
            if final_no_improve >= final_max_no_improve {
                break;
            }
            let Some(ref base) = final_ils_best_sol.clone() else {
                break;
            };
            let mut ds = match build_disj_from_solution(pre, challenge, base) {
                Ok(d) => d,
            ...
            for nd in 0..n {
                let m = ds.node_machine[nd];
                machine_loads[m] = machine_loads[m].saturating_add(ds.node_pt[nd]);
            }
            let worst_m = machine_loads
                .iter()
                .enumerate()
            ...
            for _ in 0..(k_perturb * 4) {
                if perturbed >= k_perturb || combined.is_empty() {
                    break;
                }
                fpseed ^= fpseed.wrapping_shl(13);
                fpseed ^= fpseed.wrapping_shr(7);
                fpseed ^= fpseed.wrapping_shl(17);
            ...
            let (cand_sol, cand_mk) = if let Ok(Some((ls_sol, ls_mk))) =
                critical_block_move_local_search_ex(
                    pre,
                    challenge,
                    &after_gr.0,
            ...
            if cand_mk < final_ils_best_mk {
                final_ils_best_mk = cand_mk;
                final_ils_best_sol = Some(cand_sol);
                final_no_improve = 0;
            } else {
                final_no_improve += 1;
            }
    ...
    if top_solutions.len() >= 4 {
        let vote_result3 = crossover_majority_vote(
            pre,
            challenge,
            &top_solutions,
            cb_passes + 2,
...
fn crossover_majority_vote(
    ...
    for job in 0..num_jobs {
        ...
        for op_idx in 0..num_ops {
            ...
            for (&m, &cnt) in &vote_counts[op_idx] {
                if !op_info.machines.iter().any(|&(em, _)| em == m) {
                    continue;
                }
                if cnt > best_votes {
                    best_machine = m;
    ...
    while scheduled < total_ops && stall_guard < total_ops * 6 {
        ...
        for job in 0..num_jobs {
            let op_idx = job_next_op[job];
            if op_idx >= job_machine_choices[job].len() {
                continue;
            }
        ...
        if !any {
            ...
            for j in 0..num_jobs {
                if job_ready_time[j] > time {
                    next_t = next_t.min(job_ready_time[j]);
                }
            }
            if next_t == u32::MAX {
                break;
    ...
    let ds = match build_disj_from_solution(pre, challenge, &vote_sol) {
        Ok(d) => d,
        Err(_) => return Ok(None),
    };
    let mut buf = EvalBuf::new(ds.n);
    let Some((base_mk, _)) = eval_disj(&ds, &mut buf) else {
        return Ok(None);
    };
    ...
    let (final_sol, final_mk) = if let Ok(Some((cb_sol, cb_mk))) =
        critical_block_move_local_search_ex(
            pre,
            challenge,
            &gr_sol,
    ...
    let (result_sol, result_mk) = if let Ok(Some((ecr_sol, ecr_mk))) =
        exhaustive_critical_reroute_pass(pre, challenge, &final_sol)
    {
        if ecr_mk < final_mk {
            (ecr_sol, ecr_mk)
        } else {
...
fn consensus_critical_skeleton_pass(
    pre: &Pre,
    challenge: &Challenge,
    top_solutions: &[(Solution, u32)],
    cb_passes: usize,
    ...
    let base_sol = &top_solutions[elite[0].1].0;
    let mut base_ds = build_disj_from_solution(pre, challenge, base_sol)?;
    let n = base_ds.n;
    let mcnt = base_ds.num_machines;
    if n == 0 || mcnt == 0 {
        return Ok(None);
    }

    let mut machine_votes: Vec<Vec<u16>> = vec![vec![0u16; mcnt]; n];
    let mut weighted_votes: Vec<Vec<u32>> = vec![vec![0u32; mcnt]; n];
    let mut pos_sum: Vec<Vec<u32>> = vec![vec![0u32; mcnt]; n];
    let mut pos_cnt: Vec<Vec<u16>> = vec![vec![0u16; mcnt]; n];
    let mut global_pos_sum = vec![0u32; n];
    let mut global_pos_cnt = vec![0u16; n];

    for (rank, &(_, idx)) in elite.iter().take(pool_size).enumerate() {
        let ds = match build_disj_from_solution(pre, challenge, &top_solutions[idx].0) {
            Ok(ds) => ds,
            Err(_) => continue,
        };
        if ds.n != n || ds.num_machines != mcnt {
            continue;
        }

        let rank_weight = (pool_size - rank) as u32;
        let vote_weight = rank_weight * rank_weight;
        for m in 0..mcnt {
            let seq = &ds.machine_seq[m];
            let denom = seq.len().max(1) as u32;
            for (pos, &node) in seq.iter().enumerate() {
                if node >= n {
                    continue;
                }
                let scaled_pos = ((pos as u32) * 4096) / denom;
                machine_votes[node][m] = machine_votes[node][m].saturating_add(1);
                weighted_votes[node][m] = weighted_votes[node][m].saturating_add(vote_weight);
                pos_sum[node][m] = pos_sum[node][m].saturating_add(scaled_pos);
                pos_cnt[node][m] = pos_cnt[node][m].saturating_add(1);
                global_pos_sum[node] = global_pos_sum[node].saturating_add(scaled_pos);
                global_pos_cnt[node] = global_pos_cnt[node].saturating_add(1);
            }
        }
    }

    let mut base_order_key = vec![u32::MAX; n];
    for seq in &base_ds.machine_seq {
        let denom = seq.len().max(1) as u32;
        for (pos, &node) in seq.iter().enumerate() {
            if node < n {
                base_order_key[node] = ((pos as u32) * 4096) / denom;
            }
        }
    }

    let mut best_result: Option<(Solution, u32)> = None;
    let modes = if pool_size >= 6 { 3 } else { 2 };
    for mode in 0..modes {
        let mut cand_ds = base_ds.clone();
        let mut by_machine: Vec<Vec<usize>> = vec![Vec::new(); mcnt];

        for node in 0..n {
            let job = cand_ds.node_job[node];
            let op_idx = cand_ds.node_op[node];
            let product = pre.job_products[job];
            let op = &pre.product_ops[product][op_idx];

            let base_m = base_ds.node_machine[node];
            let mut chosen_m = base_m;
            let mut chosen_votes = 0u16;
            let mut chosen_weight = 0u32;
            let mut chosen_key = u32::MAX;

            for &(m, pt) in &op.machines {
                if m >= mcnt {
                    continue;
                }
                let votes = machine_votes[node][m];
                let weight = weighted_votes[node][m];
                let key = if pos_cnt[node][m] > 0 {
                    pos_sum[node][m] / (pos_cnt[node][m] as u32)
                } else {
                    u32::MAX / 2 + pt
                };
                let better = match mode {
                    0 => votes > chosen_votes || (votes == chosen_votes && key < chosen_key),
                    1 | 2 => {
                        weight > chosen_weight
                            || (weight == chosen_weight && votes > chosen_votes)
                            || (weight == chosen_weight
                                && votes == chosen_votes
                                && key < chosen_key)
                    }
            ...
            if mode == 2 {
                let base_weight = if base_m < mcnt {
                    weighted_votes[node][base_m]
                } else {
                    0
                };
                let strong_vote = (chosen_votes as usize * 3) >= pool_size * 2;
                let strong_weight =
                    chosen_weight >= base_weight.saturating_add((pool_size as u32).max(3));
                if chosen_m != base_m && !(strong_vote || strong_weight) {
                    chosen_m = base_m;
                }
            }

            if chosen_votes == 0 && !op.machines.is_empty() {
                chosen_m = op
                    .machines
                    .iter()
                    .min_by_key(|&&(m, pt)| (pt, m))
                    .map(|&(m, _)| m)
                    .unwrap_or(chosen_m);
            }

            let Some(new_pt) = pt_from_op(op, chosen_m) else {
                return Ok(None);
            };
            cand_ds.node_machine[node] = chosen_m;
            cand_ds.node_pt[node] = new_pt;
            by_machine[chosen_m].push(node);
        }

        for m in 0..mcnt {
            by_machine[m].sort_unstable_by(|&a, &b| {
                let ak = consensus_position_key(
                    a,
                    m,
                    &pos_sum,
                    &pos_cnt,
                ...
                let a_key = if mode == 2 && pos_cnt[a][m] < 2 {
                    base_order_key[a]
                } else {
                    ak
                };
                let b_key = if mode == 2 && pos_cnt[b][m] < 2 {
                    base_order_key[b]
                } else {
                    bk
                };
                a_key
                    .cmp(&b_key)
                    .then_with(|| ak.cmp(&bk))
                    .then_with(|| cand_ds.node_op[a].cmp(&cand_ds.node_op[b]))
                    .then_with(|| cand_ds.node_job[a].cmp(&cand_ds.node_job[b]))
            });
        }
        cand_ds.machine_seq = by_machine;

        let mut buf = EvalBuf::new(n);
        let Some((skeleton_mk, _)) = eval_disj(&cand_ds, &mut buf) else {
            continue;
        };
        let skeleton_sol = match disj_to_solution(pre, &cand_ds, &buf.start) {
            Ok(s) => s,
            Err(_) => continue,
        };

        let (mut cand_sol, mut cand_mk) = match greedy_reassign_pass(pre, challenge, &skeleton_sol)
        {
            Ok(Some((s, mk))) if mk < skeleton_mk => (s, mk),
            _ => (skeleton_sol, skeleton_mk),
        };
        ...
        if let Ok(Some((anti_sol, anti_mk))) =
            false_consensus_critical_flip_pass(pre, challenge, &cand_sol, 24)
        {
            if anti_mk < cand_mk {
                cand_sol = anti_sol;
                cand_mk = anti_mk;
        ...
        if let Ok(Some((ecr_sol, ecr_mk))) =
            exhaustive_critical_reroute_pass(pre, challenge, &cand_sol)
        {
            if ecr_mk < cand_mk {
                cand_sol = ecr_sol;
                cand_mk = ecr_mk;
        ...
        if best_result
            .as_ref()
            .map_or(true, |(_, best_mk)| cand_mk < *best_mk)
        {
            best_result = Some((cand_sol, cand_mk));
        }
    }
...
fn consensus_position_key(
    ...
    if pos_cnt[node][machine] > 0 {
        pos_sum[node][machine] / (pos_cnt[node][machine] as u32)
    } else if global_pos_cnt[node] > 0 {
        global_pos_sum[node] / (global_pos_cnt[node] as u32)
    } else {
        u32::MAX / 2
    }
}

fn false_consensus_critical_flip_pass(
    pre: &Pre,
    challenge: &Challenge,
    base_sol: &Solution,
    max_trials: usize,
    ...
    let mut ds = build_disj_from_solution(pre, challenge, base_sol)?;
    let mut buf = EvalBuf::new(ds.n);
    let Some((mut best_mk, mk_node)) = eval_disj(&ds, &mut buf) else {
        return Ok(None);
    };
    ...
    for m in 0..ds.num_machines {
        ...
        let mut i = 0usize;
        while i + 1 < seq_len && trials < max_trials {
            let a = ds.machine_seq[m][i];
            let b = ds.machine_seq[m][i + 1];
            if crit[a] && crit[b] && ds.node_job[a] != ds.node_job[b] {
                ds.machine_seq[m].swap(i, i + 1);
                if let Some((mk, _)) = eval_disj(&ds, &mut buf) {
                    if mk < best_mk {
                        best_mk = mk;
                        best_ds = ds.clone();
                    }
    ...
    let Some((final_mk, _)) = eval_disj(&best_ds, &mut buf) else {
        return Ok(None);
    };
...
fn bottleneck_machine_relief_pass(
    ...
    if !any_improvement || current_mk >= initial_mk {
        return Ok(None);
    }
    let Some((_, _)) = eval_disj(&ds, &mut buf) else {
        return Ok(None);
