...
fn solution_machine_signature(pre: &Pre, challenge: &Challenge, sol: &Solution) -> Vec<u16> {
    ...
    for job in 0..challenge.num_jobs {
        ...
        for op_idx in 0..pre.job_ops_len[job] {
            sig.push(if op_idx < lim {
                sol.job_schedule[job][op_idx].0 as u16
            } else {
                u16::MAX
            });
        }
...
fn push_top_solutions_diverse(
    ...
    while pool.len() > cap {
        ...
        for i in 0..len {
            if pool[i].1 == worst_mk && min_nn[i] < drop_min_nn {
                drop_idx = i;
                drop_min_nn = min_nn[i];
            }
        }
...
fn consensus_learning_from_elites(
    ...
    for (sol, mk, _) in elites {
        ...
        for job in 0..challenge.num_jobs {
            ...
            if prod >= pre.product_ops.len() {
                continue;
            }
            let sched = &sol.job_schedule[job];
            let lim = sched.len().min(pre.product_ops[prod].len());
            for op_idx in 0..lim {
                let (m, st) = sched[op_idx];
                if m >= challenge.num_machines {
                    continue;
                }
                let pt = pre.product_ops[prod][op_idx]
                    .machines
                    .iter()
                    .find(|&&(mm, _)| mm == m)
                    .map(|&(_, p)| p)
    ...
    let mut counts: Vec<Vec<Vec<u32>>> = pre
        ...
        .map(|ops| {
            ops.iter()
                .map(|_| vec![0u32; challenge.num_machines])
                .collect()
        })
        .collect();
    for (sol, _, _) in elites {
...
fn cp_window_exhaustive(
    ...
    for _ in 0..max_iters {
        ...
        for nd in 0..n {
            if buf.start[nd].saturating_add(ds.node_pt[nd]) == cur_mk {
                on_cp[nd] = true;
            }
        }
        ...
        for &nd in &order {
            ...
            if let Some(pos) = seq.iter().position(|&x| x == nd) {
                if pos > 0 {
                    let p = seq[pos - 1];
                    if buf.start[p].saturating_add(ds.node_pt[p]) == buf.start[nd] {
                        on_cp[p] = true;
                    }
                }
            }
        ...
        'ws: for &wsize in &[4usize, 3, 2] {
            let slen = ds.machine_seq[bn_m].len();
            if slen < wsize {
                continue;
            }
            for start in 0..=(slen - wsize) {
                if !ds.machine_seq[bn_m][start..start + wsize]
                    .iter()
                    .any(|&nd| on_cp[nd])
                {
                    continue;
                ...
                for i in 0..wsize {
                    orig[i] = ds.machine_seq[bn_m][start + i];
                }
                let mut best_pmk = cur_mk;
                ...
                for pi in 0..pc {
                    ...
                    for i in 0..wsize {
                        ds.machine_seq[bn_m][start + i] = orig[p[i]];
                    }
                ...
                if let Some(p) = best_p {
                    ...
                    for i in 0..wsize {
                        ds.machine_seq[bn_m][start + i] = orig[i];
                    }
                }
            }
        }
...
fn iterative_cp_descent(
    ...
    for _iter in 0..max_iters {
        ...
        for nd in 0..n {
            finish[nd] = buf.start[nd].saturating_add(ds.node_pt[nd]);
        }
        let makespan = current_mk;

        let mut on_cp = vec![false; n];
        for nd in 0..n {
            if finish[nd] == makespan {
                on_cp[nd] = true;
            }
        }
        ...
        for &nd in &order {
            ...
            if op_idx > 0 {
                ...
                if pred_op < job_op_to_node[job].len() {
                    ...
                    if pred != usize::MAX && finish[pred] == buf.start[nd] {
                        on_cp[pred] = true;
                    }
                }
            }
        ...
        let mut cp_flex: Vec<usize> = (0..n)
            .filter(|&nd| {
                ...
                let op_idx = ds.node_op[nd];
                let product = pre.job_products[job];
                op_idx < pre.product_ops[product].len()
        ...
        for &nd in &cp_flex {
            ...
            if best_m != cur_m {
                ...
                ds.node_machine[nd] = best_m;
                ds.node_pt[nd] = best_pt;
                current_mk = best_mk;
                let _ = eval_disj(&ds, &mut buf);
                iter_improved = true;
            }
        ...
        if cp_flex.len() >= 2 {
            ...
            let cand_positions =
                |ds: &DisjSchedule, buf: &EvalBuf, m: usize, nd: usize| -> Vec<usize> {
                    let cur_start = buf.start[nd];
                                    ...
                                    for &pbi in &pb {
                                        ...
                                        if let Some((tmk, _)) = eval_disj(&ds, &mut buf) {
                                            if tmk < best_mk_pair {
                                                ...
                                                if order_flag == 0 {
                                                    best_config =
                                                        Some((m, pt1, pai, m, pt2, pbi, 0));
                                                } else {
                                                    best_config =
                                                        Some((m, pt1, pbi, m, pt2, pai, 1));
                                                }
                                            }
                                        }
...
pub fn solve(
    ...
    let mut best_sol = greedy_sol.clone();
    ...
    let mut rb = vec![u32::MAX; 10];
    let mut rt = vec![0u32; 10];
    for (rr, mk, _) in &ranked {
        let i = local_rule_idx(*rr);
        rb[i] = rb[i].min(*mk);
        rt[i] += 1;
    }

    let (jb0, mp0, rp0) = consensus_learning_from_elites(pre, challenge, &pool)?;
    let mut ljb = Some(jb0);
    let mut lmp = Some(mp0);
    let mut lrp = Some(rp0);
    let mut lu = 10usize;
    let tm: u32 =
    ...
    // PHASE 1: Construction restarts with INLINE Schrage + Window improvement.
    // v6 fix: bench at n=50/5T showed `num_restarts=500` (v4 author's claim of
    // +1,652 quality at 10 nonces) actually loses 3,553 quality vs job_eight's
    // default of 5,000 — v4/v5 produced 55,178 on fjsp_high while job_eight
    // standalone produced 58,731. The earlier "500 wins" tuning was for a
    // different fuel cap. At the current 5T cap with n=50, refinement phases
    // 2-8 don't compensate for the under-construction, so restoring 5,000
    // matches the on-chain job_eight / near_jsp_v3 quality of 58,731.
    let num_restarts = 5000;
    for r in 0..num_restarts {
        let p = r as f64 / num_restarts as f64;
        let late = p >= 0.66;
        let (kn, kx) = if stuck > 170 {
            (4, 6usize.min(kh))
        } else if stuck > 90 {
            (3, 6usize.min(kh))
        } else if stuck > 35 {
        ...
        let lp = (0.08 + 0.22 * pre.jobshopness + 0.18 * pre.high_flex).clamp(0.05, 0.42)
            * (1.0 + 0.35 * ((stuck as f64) / 120.0).clamp(0.0, 1.0));
        let ul = ljb.is_some() && lmp.is_some() && rng.gen::<f64>() < lp && lrp.is_some();
        let tgt = if best_mk < u32::MAX / 2 {
            Some(best_mk.saturating_add(tm))
        } else {
            None
        };
        let (sol, mk) = if ul {
            ...
            construct_solution_conflict(
                challenge, pre, rule, k, tgt, &mut rng, None, None, None, 0.0,
            )?
        };
        let ri = local_rule_idx(rule);
        rt[ri] += 1;
        rb[ri] = rb[ri].min(mk);
        push_top_solutions_diverse(pre, challenge, &mut pool, &sol, mk, 15);
        if mk < best_mk {
            ...
            if lu > 0 {
                if let Ok((j, m, rp)) = consensus_learning_from_elites(pre, challenge, &pool) {
                    ljb = Some(j);
                    lmp = Some(m);
                    lrp = Some(rp);
                    lu -= 1;
    ...
    for i in 0..er {
        let bs = &pool[i].0;
        let jb = job_bias_from_solution(pre, bs)?;
        let mp = machine_penalty_from_solution(pre, bs, challenge.num_machines)?;
        let rp = Some(route_pref_from_solution_lite(pre, bs, challenge)?);
        let tl = if best_mk < u32::MAX / 2 {
            Some(best_mk.saturating_add(tm / 2))
        } else {
            None
        };
        for a in 0..10 {
    ...
    if let Some((s, m)) =
        critical_block_move_local_search_ex(pre, challenge, &best_sol, 10, 80, 14)?
    {
        if m < best_mk {
            best_mk = m;
            best_sol = s.clone();
