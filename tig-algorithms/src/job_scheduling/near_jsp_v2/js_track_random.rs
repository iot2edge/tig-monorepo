...
use super::js_construction::{construct_solution_conflict, neh_reentrant_flow_solution};
use super::js_helpers::push_top_solutions;
use super::js_learning::{
    job_bias_from_solution, machine_penalty_from_solution, route_pref_from_solution_lite,
};
use super::js_local_search::{critical_block_move_local_search, critical_block_tabu_search};
use super::js_rules::{choose_rule_bandit, rule_idx};
use super::js_types::*;
...
pub fn solve(
    ...
    // Final polish: deeper LS pass on the best solution found.
    if let Some(ref sol) = best_solution {
        if let Some((sol2, mk2)) =
            critical_block_move_local_search(&pre, challenge, sol, max_iters * 2, top_cands * 2)?
        {
            if mk2 < best_makespan {
                best_makespan = mk2;
                best_solution = Some(sol2.clone());
    ...
    // NS-style tabu post-pass: only run once on the very best solution to push
    // past local optima the descent + perturb stack got stuck in.
    if let Some(ref sol) = best_solution {
        let tabu_iters = 800usize;
        if let Some((sol2, mk2)) =
            critical_block_tabu_search(&pre, challenge, sol, tabu_iters, top_cands * 2, 10)?
        {
            if mk2 < best_makespan {
                best_makespan = mk2;
                best_solution = Some(sol2.clone());
