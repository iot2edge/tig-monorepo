...
fn run_one_instance(challenge: &Challenge, hp: &Hparams, rng_offset: usize) -> Solution {
    ...
    for variant in 0..4 {
        ...
        match variant {
            ...
            _ => build_greedy_synergy_weight(&mut st),
        }
    ...
    let max_ils_rounds = hp.ils_rounds;

    for round in 0..max_ils_rounds {
        let snap = state.clone_solution();

        dp_refinement(&mut state);
        ils_vnd(&mut state, hp);
...
pub fn help() {
    println!("near_knap_improve: QKP solver with diversified construction, ILS, and VND polish.");
}
