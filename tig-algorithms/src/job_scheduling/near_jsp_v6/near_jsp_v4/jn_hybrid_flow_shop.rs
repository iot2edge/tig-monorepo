    ...
    while remaining_ops > 0 {
        loop {
            ...
            for job in 0..num_jobs {
                ...
                let density_n =
                    ((rem_min / (ops_rem as f64).max(1.0)) / avg_op_min_scale).clamp(0.0, 4.0);
                ...
                for &(m, pt) in &op.machines {
                    ...
                    let base = match rule {
                        ...
                        Rule::ShortestProc => {
                            (0.0 - proc_u) + rem_min_u * (1.0 + next_u) + sat_scarcity + base_bias
                                - end_u
                                - pop_pen
                                + route_term
                        }
                        ...
                        Rule::BnHeavy => {
                            ...
                            bn_focus + chain + scarce_slack + reg_u + flex_u + base_bias
                                - penal
                                - pop_pen
                                + route_term
                        }
            ...
            let mut top: Vec<Cand> = if ek > 0 {
                Vec::with_capacity(ek)
            } else {
                Vec::new()
            };
...
pub fn solve(
    ...
    for i in 0..ranked.len().min(3) {
        ...
        elite_mp.push(machine_penalty_from_solution(
            pre,
            sol,
            challenge.num_machines,
        )?);
    ...
    for r in 0..num_restarts {
        ...
        let use_elite_pool = !elite_mk.is_empty()
            && r > 10
            && rng.gen::<f64>() < (learn_base2 * learn_boost2).clamp(0.0, 0.65);
        ...
        let (sol, mk) = if use_elite_pool {
            ...
            let mix_p =
                (0.055 + 0.10 * pre.high_flex + 0.09 * pre.jobshopness + 0.16 * elite_boost)
                    .clamp(0.05, 0.40);
            let mp_idx = if n > 1 && rng.gen::<f64>() < mix_p {
                pick_elite(&mut rng)
            } else {
                base_idx
            };
            let rp_idx = if n > 1 && rng.gen::<f64>() < mix_p {
                pick_elite(&mut rng)
            } else {
                base_idx
            };
            ...
            construct_hfs_sig(
                ...
                target,
                &mut rng,
                Some(&elite_jb[base_idx]),
                Some(&elite_mp[mp_idx]),
                Some(&elite_rp[rp_idx]),
                route_w,
            )?
    ...
    // Pass D: Final CBMLS
    if let Some(ref sol) = best_solution {
        if let Some((s, m)) = critical_block_move_local_search_ex(
            pre, challenge, sol, ls_iters, ls_cands, ls_perturb,
        )? {
            if m < best_makespan {
                best_makespan = m;
                best_solution = Some(s.clone());
            ...
            while std::time::Instant::now() < phase_h1_deadline {
                if loop_iters > 0 && loop_iters % 50 == 0 {
                    if let Some(ref best) = best_solution {
                        current_base = best.clone();
                    }
                }
                loop_iters += 1;

                let rate = if no_improve_count > 20 {
                    0.40
                } else {
                    perturb_rates[rate_idx % perturb_rates.len()]
                };
                rate_idx += 1;
                ...
                for j in 0..num_jobs {
                    if target_stage >= pre.job_ops_len[j] {
                        continue;
                    }
                    if rng.gen::<f64>() >= rate {
                        continue;
                ...
                if let Some((ref quick_sol, quick_mk)) = quick_res {
                    if quick_mk < best_makespan {
                        ...
                        if target_stage < stage_weight.len() {
                            stage_weight[target_stage] =
                                (stage_weight[target_stage] * 2.0).min(20.0);
                ...
                if !run_ses && rng.gen::<f64>() < 0.15 {
                    run_ses = true;
                }

                if run_ses {
                    if let Ok(Some((healed_sol, healed_mk))) =
                        slack_ejection_schrage(pre, challenge, ses_input_ref, 30)
                    {
                        ...
                        if healed_mk < best_makespan {
                            ...
                            if target_stage < stage_weight.len() {
                                stage_weight[target_stage] =
                                    (stage_weight[target_stage] * 1.5).min(20.0);
                                stage_fail_count[target_stage] = 0;
                            }
                        } else {
                            ...
                            if target_stage < stage_fail_count.len() {
                                ...
                                if stage_fail_count[target_stage] % 30 == 0 {
                                    stage_weight[target_stage] =
                                        (stage_weight[target_stage] * 0.85).max(0.2);
                                }
                            }
                        }
                        ...
                        if improved {
                            if let Ok(Some((deep_sol, deep_mk))) =
                                slack_ejection_schrage(pre, challenge, &healed_sol, 30)
                            {
    ...
    // creating genuinely new entry points. Also use top-50 seeds (wider pool).
    {
        ...
        let top_seeds: Vec<Solution> = top_solutions
            .iter()
        ...
        if !top_seeds.is_empty() {
            ...
            while std::time::Instant::now() < phase_h2_deadline {
                ...
                if let Ok(Some((improved_sol, improved_mk))) =
                    slack_ejection_schrage(pre, challenge, &effective_seed, 30)
                {
                    ...
                    if improved_mk < best_makespan.saturating_add(50) {
                        if let Ok(Some((healed_sol, healed_mk))) =
                            schrage_pass(pre, challenge, &improved_sol, 5)
                        {
                            if healed_mk < best_makespan {
                                best_makespan = healed_mk;
                                best_solution = Some(healed_sol.clone());
    ...
    // operations (agreement < 85%). Much smaller perturbations → within RTT tolerance.
    {
        ...
        let mut machine_freq: Vec<Vec<u32>> =
            vec![vec![0u32; challenge.num_machines]; pre.total_ops];
        ...
        let job_op_offsets: Vec<usize> = {
            ...
            for j in 0..num_jobs {
                off[j + 1] = off[j] + pre.job_ops_len[j];
            }
            off
        };
        ...
        for (sol, _) in &top_solutions {
            for j in 0..num_jobs {
                ...
                if j >= sol.job_schedule.len() {
                    continue;
                }
                let op_base = job_op_offsets[j];
                for stage in 0..jops {
                    if stage >= sol.job_schedule[j].len() {
                        continue;
                    }
        ...
        for j in 0..num_jobs {
            ...
            for stage in 0..jops {
                let op_id = op_base + stage;
                if op_id >= machine_freq.len() {
                    continue;
                }
                if product >= pre.product_ops.len() || stage >= pre.product_ops[product].len() {
                    continue;
                }
                if pre.product_ops[product][stage].machines.len() < 2 {
                    continue;
                }
            ...
            for j in 0..num_jobs {
                ...
                for stage in 0..pre.job_ops_len[j] {
                    if product < pre.product_ops.len()
                        && stage < pre.product_ops[product].len()
        ...
        let init_sol = if let Some(ref s) = best_solution {
            s.clone()
        } else if !top_solutions.is_empty() {
            top_solutions[0].0.clone()
        } else {
            return Ok(());
        };
        ...
        while std::time::Instant::now() < lns_deadline {
            ...
            for i in 0..n_perturb {
                ...
                if j >= candidate.job_schedule.len() || stage >= candidate.job_schedule[j].len() {
                    continue;
                }
                let product = pre.job_products[j];
                if product >= pre.product_ops.len() || stage >= pre.product_ops[product].len() {
            ...
            if repaired_mk < best_makespan {
                ...
                if no_improve_streak >= no_improve_streak_limit {
                    lns_current = best_solution
                        .clone()
                        .unwrap_or_else(|| top_solutions[0].0.clone());
                    no_improve_streak = 0;
                }
            }
        }
    ...
    // Multi-start Pass TS: N1 tabu on best + diverse elite solutions
    {
        ...
        for ts_base in &ts_pool {
            if let Ok(Some((ts_sol, ts_mk))) =
                super::jn_job_shop::tabu_search_phase(pre, challenge, ts_base, ts_budget, ts_tenure)
            {
    ...
    if let Some(ref base) = best_solution.clone() {
        if let Some((scd_sol, scd_mk)) = scd_sr_pass(pre, challenge, base, best_makespan, 50) {
            if scd_mk < best_makespan {
                ...
                let ts_tenure = ((pre.total_ops as f64).sqrt() as usize).max(5).min(20);
                if let Ok(Some((ts_sol, ts_mk))) = super::jn_job_shop::tabu_search_phase(
                    pre, challenge, &scd_sol, ts_budget, ts_tenure,
                ) {
                    if ts_mk < best_makespan {
                        best_makespan = ts_mk;
                        best_solution = Some(ts_sol.clone());
    ...
    if let Some(ref base) = best_solution.clone() {
        let (ils_sol, ils_mk) =
            fast_ils_hfs(pre, challenge, base, best_makespan, &mut rng, save_solution);
    ...
    // Multi-start SCD-SR from elite pool (3 restarts, 50 sweeps, ~75s budget)
    {
        ...
        if let Some(ref s) = best_solution {
            pool_starts.push((s.clone(), best_makespan));
        }
        for (sol, mk) in top_solutions.iter().take(5) {
            if pool_starts.len() >= 3 {
                break;
            }
...
fn scd_sr_pass(
    ...
    for _sweep in 0..max_sweeps {
        ...
        for s in 0..max_stages {
            ...
            if (0..num_jobs).all(|j| s >= pre.job_ops_len[j]) {
                break;
            }
            ...
            let mut stage_jobs: Vec<usize> =
                (0..num_jobs).filter(|&j| s < pre.job_ops_len[j]).collect();
            ...
            for j in 0..num_jobs {
                for t in 0..s.min(pre.job_ops_len[j]) {
                    ...
                    if pt > 0 {
                        insert_usage(&mut machine_usages[m], st, st + pt);
                    }
                }
            }
            ...
            for t in (s + 1)..max_stages {
                if (0..num_jobs).all(|j| t >= pre.job_ops_len[j]) {
                    break;
                }
...
fn tabu_ils_hfs(
    ...
    for iter in 0..max_iters {
        if no_improve >= stagnation_limit {
            ...
            ds = best_ds.clone();
            let Some((mk, _)) = eval_disj(&ds, &mut buf) else {
                break;
            };
            cur_mk = mk;
            no_improve = 0;
        ...
        for m in 0..ds.num_machines {
            ...
            while i < ds.machine_seq[m].len() {
                ...
                if bend > bstart {
                    ...
                    let swap_positions = if block_len >= 3 {
                        [bstart, bend - 1]
                    } else {
                        [bstart, NONE_USIZE]
                    };
        ...
        if iter % 3 == 0 {
            for node in 0..n {
                ...
                for &(new_m, new_pt) in &op_info.machines {
                    ...
                    let positions =
                        find_insert_positions_hfs(&ds, &buf.start, node, new_m, &job_pred_node);
                    for insert_pos in positions {
                        ...
                        if (!is_tabu || aspiration) && est_mk < best_move_mk {
                            best_move_mk = est_mk;
                            best_move = Some(MoveHfs::Reassign {
                                node,
                                new_machine: new_m,
                                new_pt,
                                insert_pos,
                        ...
                        if est_mk < fallback_mk {
                            fallback_mk = est_mk;
                            fallback_move = Some(MoveHfs::Reassign {
                                node,
                                new_machine: new_m,
                                new_pt,
                                insert_pos,
...
impl SbsWorkspace {
    ...
    fn clear(&mut self) {
        ...
        self.order.clear();
        for u in self.usages.iter_mut() {
            u.clear();
        }
    }
}

fn eval_sbs_makespan(
    seq: &[usize],
    pre: &Pre,
    challenge: &Challenge,
    wk: &mut SbsWorkspace,
    ...
    wk.clear();
    for (idx, &j) in seq.iter().enumerate() {
        wk.seq_rank[j] = idx as u32;
    }
    wk.order.extend_from_slice(seq);
    if wk.order.is_empty() {
        return (0, 0);
    }
...
pub fn decode_hfs_gap_filling(seq: &[usize], pre: &Pre, challenge: &Challenge) -> (Solution, u32) {
    let mut usage =
        vec![Vec::<(u32, u32)>::with_capacity(challenge.num_jobs); challenge.num_machines];
    let mut schedule: Vec<Vec<(usize, u32)>> =
        vec![Vec::with_capacity(pre.max_ops); challenge.num_jobs];
...
fn decode_perm_to_solution(seq: &[usize], pre: &Pre, num_machines: usize) -> (Solution, u32) {
    let mut machine_ready = vec![0u32; num_machines];
    let mut job_schedule: Vec<Vec<(usize, u32)>> = seq
        .iter()
        .map(|_| Vec::with_capacity(pre.max_ops))
        .collect();
    let mut makespan = 0u32;
    for &j in seq {
        let p = pre.job_products[j];
...
fn dynamic_memetic_ig(
    ...
    loop {
        if loop_counter % 10 == 0 && std::time::Instant::now() >= deadline {
            break;
        }
        loop_counter += 1;
        ...
        if trial_mk < best_mk {
            ...
            *global_best_mk = (*global_best_mk).min(best_mk);
            ...
            d = if stuck_iters > 150 {
                current_seq.clear();
                current_seq.extend_from_slice(&best_seq);
                current_mk = best_mk;
                stuck_iters = 0;
                4
            } else if stuck_iters > 100 {
                12
            } else if stuck_iters > 50 {
                8
            } else {
                4
            };
