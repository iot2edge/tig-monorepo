...
pub fn local_search_vnd(state: &mut State, params: &Params) {
    ...
    let n = state.ch.num_items;
    let base_max_iterations = if n >= 4500 {
        180
    } else if n >= 3000 {
        260
    } else if n >= 1000 {
    ...
    let max_iterations = params.vnd_iter_override.unwrap_or(base_max_iterations);
    ...
    loop {
        ...
        if (iterations & 1) == 0 && apply_swap_2_2_bounded(state, swap_22_k) {
            dirty_window = true;
            continue;
        }
...
fn apply_swap_2_2_bounded(state: &mut State, k: usize) -> bool {
    ...
    let mut unsel_ranked: Vec<(usize, i32)> = (0..n)
        ...
        .map(|i| (i, state.contrib[i]))
        .collect();
    unsel_ranked.sort_unstable_by_key(|&(_, c)| core::cmp::Reverse(c));
