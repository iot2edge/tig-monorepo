...
impl TrackHp {
    pub fn override_from_map(&mut self, h: &Option<Map<String, Value>>) {
        ...
        if let Some(v) = m.get("soc_levels").and_then(|v| v.as_u64()) {
            self.soc_levels = (v as usize).max(3);
        }
        if let Some(v) = m.get("action_grid").and_then(|v| v.as_u64()) {
            self.action_grid = (v as usize).max(4);
        }
        if let Some(v) = m.get("asca_iters").and_then(|v| v.as_u64()) {
            self.asca_iters = v as usize;
        }
        if let Some(v) = m.get("ternary_iters").and_then(|v| v.as_u64()) {
            self.ternary_iters = v as usize;
        }
        if let Some(v) = m.get("convergence_tol").and_then(|v| v.as_f64()) {
            self.convergence_tol = v;
        }
        if let Some(v) = m.get("anticipate_lmp").and_then(|v| v.as_bool()) {
            self.anticipate_lmp = v;
        }
        if let Some(v) = m.get("lmp_threshold").and_then(|v| v.as_f64()) {
            self.lmp_threshold = v;
        }
        if let Some(v) = m.get("lmp_premium_scale").and_then(|v| v.as_f64()) {
            self.lmp_premium_scale = v;
        }
        if let Some(v) = m.get("jump_premium").and_then(|v| v.as_f64()) {
            self.jump_premium = v;
        }
        if let Some(v) = m.get("prune_ratio").and_then(|v| v.as_f64()) {
            self.prune_ratio = v.clamp(0.0, 0.9);
        }
        if let Some(v) = m.get("deflator_iters").and_then(|v| v.as_u64()) {
            self.deflator_iters = v as usize;
        }
        if let Some(v) = m.get("flow_margin").and_then(|v| v.as_f64()) {
            self.flow_margin = v.max(0.0);
        }
        if let Some(v) = m.get("network_derating").and_then(|v| v.as_f64()) {
            self.network_derating = v.clamp(0.01, 1.0);
        }
    }
}
...
struct Inner {
    ...
    // RT-DA drift correction state.
    nodal_drift: Vec<f64>,    // EWMA of (rt_price - da_price) per node
    drift_count: usize,       // number of observations
    last_rebuild_step: usize, // step at which cache was last (re)built
}
...
pub fn solve_with_hp(
    ...
    STATE.with(|s| {
        *s.borrow_mut() = Some(Inner {
            hp,
            cache: None,
            nodal_drift: vec![0.0_f64; challenge.network.num_nodes],
            drift_count: 0,
            last_rebuild_step: 0,
        })
    });
...
fn policy_entry(challenge: &Challenge, state: &State) -> Result<Vec<f64>> {
    STATE.with(|s| -> Result<Vec<f64>> {
        ...
        // Multi-start ASCA: run from multiple starting points and keep the
        // best by DP-evaluated total value. Each pass is cheap relative to the
        // 5T fuel cap (we use 1-4% of cap).
        //
        // Pass A: titan default — start from zeros.
        let mut actions_a = vec![0.0_f64; num_b];
        run_asca(challenge, state, cache, hp, &flows_base, &mut actions_a);
        run_deflator(challenge, state, cache, hp, &flows_base, &mut actions_a);

        // Pass B: greedy-individual seed. For each battery, probe a few action
        // candidates (u_min, 0, u_max, half-range) and start from the
        // DP-optimal unconstrained one. ASCA then enforces flow constraints.
        let mut actions_b = vec![0.0_f64; num_b];
        for b in 0..num_b {
            let (lo, hi) = state.action_bounds[b];
            let mut best_u = 0.0_f64;
            let mut best_v = eval_profit(challenge, state, cache, b, 0.0);
            for &u_try in &[lo, hi, 0.5 * lo, 0.5 * hi] {
                let v = eval_profit(challenge, state, cache, b, u_try);
                if v > best_v {
                    best_v = v;
                    best_u = u_try;
                }
        ...
        run_asca(challenge, state, cache, hp, &flows_base, &mut actions_b);
        run_deflator(challenge, state, cache, hp, &flows_base, &mut actions_b);

        // Pass C: half-strength of pass-B seed. Different basin from full
        // greedy seed — captures situations where pass-B over-commits.
        let mut actions_c = vec![0.0_f64; num_b];
        for b in 0..num_b {
            actions_c[b] = 0.5 * actions_b[b];
        }
        run_asca(challenge, state, cache, hp, &flows_base, &mut actions_c);
        run_deflator(challenge, state, cache, hp, &flows_base, &mut actions_c);

        // Pick the best of {A, B, C} by summed DP-evaluated value.
        let val_a: f64 = (0..num_b)
            .map(|b| eval_profit(challenge, state, cache, b, actions_a[b]))
            .sum();
        let val_b: f64 = (0..num_b)
            .map(|b| eval_profit(challenge, state, cache, b, actions_b[b]))
            .sum();
        let val_c: f64 = (0..num_b)
            .map(|b| eval_profit(challenge, state, cache, b, actions_c[b]))
            .sum();
        let (mut best_actions, _) = [(actions_a, val_a), (actions_b, val_b), (actions_c, val_c)]
            .into_iter()
            .max_by(|x, y| x.1.partial_cmp(&y.1).unwrap_or(std::cmp::Ordering::Equal))
            .unwrap();

        // ALGORITHMIC BREAKTHROUGH: post-ASCA joint pair refinement.
        // ASCA converges to a per-battery Nash equilibrium — no single battery
        // wants to deviate alone. But pairs of batteries with offsetting flow
        // impacts can both improve simultaneously. We try pair swaps of the
        // form (Δ on b1, -k·Δ on b2) where k matches their PTDF impact ratio
        // on a chosen line, preserving line flow while shifting profit.
        joint_pair_refine(challenge, state, cache, hp, &flows_base, &mut best_actions);

        Ok(best_actions)
    })
}

fn build_cache(challenge: &Challenge, state: &State, hp: &TrackHp, drift: &[f64]) -> TitanCache {
    let num_b = challenge.num_batteries;
    ...
    for b in 0..num_b {
        ...
        for t in (0..num_t).rev() {
            ...
            // Apply RT-DA drift correction if we have an estimate (mid-rollout
            // rebuild). When `drift` is empty (initial build), this is a no-op.
            let p_da = p_da_raw + if node < drift.len() { drift[node] } else { 0.0 };
...
fn run_asca(
    ...
    if hp.prune_ratio > 0.0 && num_b >= 2 {
        ...
        if cutoff > 0 {
            let mut caps: Vec<(usize, f64)> = challenge
                .batteries
                .iter()
                .enumerate()
                .map(|(i, b)| (i, b.capacity_mwh))
                .collect();
            caps.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap_or(std::cmp::Ordering::Equal));
            for i in 0..cutoff.min(num_b) {
                active[caps[i].0] = false;
            }
        }
    }
    ...
    for _sweep in 0..hp.asca_iters {
        ...
        for &b in &order {
            ...
            if u_min <= 0.0 && 0.0 <= u_max && v0 > best_v {
                best_v = v0;
                best_u = 0.0;
            }
...
/// Post-ASCA joint pair refinement (algorithmic breakthrough).
///
/// ASCA's coordinated descent reaches a per-battery local optimum — no single
/// battery wants to unilaterally deviate. But pairs of batteries with
/// non-trivial PTDF coupling can both improve simultaneously: e.g., increase
/// battery b1 by Δ and decrease battery b2 by k·Δ where k matches their PTDF
/// impacts on the binding line, preserving line flow while moving profit.
///
/// We enumerate candidate pairs sharing a common line, try a small ternary
/// search over the joint Δ scaling, and accept improvements that stay
/// flow-feasible. O(num_lines × max_pair × ternary_iters) extra work per step.
fn joint_pair_refine(
    challenge: &Challenge,
    state: &State,
    ca: &TitanCache,
    hp: &TrackHp,
    ...
    // For each line, find pairs of batteries that share it (both have
    // non-trivial PTDF on the line). Try joint moves on each pair.
    let max_pairs_per_line = 4;
    for l in 0..num_l {
        let line_batts: Vec<(usize, f64)> = ca.ptdf_sparse[l]
            .iter()
            .filter(|(_, p)| p.abs() > 1e-6)
            .copied()
            .collect();
        if line_batts.len() < 2 {
            continue;
        ...
        // Sort by absolute PTDF impact descending — larger-impact batteries
        // first since they have more leverage.
        let mut sorted = line_batts.clone();
        sorted.sort_by(|a, b| {
            b.1.abs()
                .partial_cmp(&a.1.abs())
        ...
        for i in 0..sorted.len() {
            ...
            for j in (i + 1)..sorted.len() {
                ...
                // Choose Δ on b1, then to keep this line's flow contribution
                // from this pair constant, set Δ_b2 = -(p1 / p2) · Δ.
                let k = -p1 / p2;

                let (lo1, hi1) = state.action_bounds[b1];
                let (lo2, hi2) = state.action_bounds[b2];

                // Find Δ range such that both new actions stay within bounds.
                let a1 = actions[b1];
                let a2 = actions[b2];
                let mut d_lo = (lo1 - a1).max((hi2 - a2) / k.min(-1e-12));
                let mut d_hi = (hi1 - a1).min((lo2 - a2) / k.min(-1e-12));
                if k > 0.0 {
                    // Different orientation; recompute properly.
                    d_lo = (lo1 - a1).max((lo2 - a2) / k.max(1e-12));
                    d_hi = (hi1 - a1).min((hi2 - a2) / k.max(1e-12));
                }
                if d_lo >= d_hi {
                    continue;
                }

                // Evaluate joint profit as a function of Δ. Ternary search.
                let pair_val = |delta: f64| -> f64 {
                    let new_a1 = a1 + delta;
                    let new_a2 = a2 + k * delta;
                    let v1 = eval_profit(challenge, state, ca, b1, new_a1);
                    let v2 = eval_profit(challenge, state, ca, b2, new_a2);
                    v1 + v2
                };

                let v0 = pair_val(0.0);
                let (delta_best, v_best) =
                    ternary_search(pair_val, d_lo, d_hi, hp.ternary_iters.max(8));

                if v_best <= v0 + 1e-9 {
                    continue;
                }

                // Check that the joint move keeps ALL lines feasible.
                let new_a1 = a1 + delta_best;
                let new_a2 = a2 + k * delta_best;
                let delta_b1 = delta_best;
                let delta_b2 = k * delta_best;

                let mut feasible = true;
                for ll in 0..num_l {
                    let mut imp_b1 = 0.0_f64;
                    let mut imp_b2 = 0.0_f64;
                    for &(b, p) in &ca.ptdf_sparse[ll] {
                        if b == b1 {
                            imp_b1 = p;
                        }
                        if b == b2 {
                    ...
                    let new_flow = flows[ll] + imp_b1 * delta_b1 + imp_b2 * delta_b2;
                    let limit = (challenge.network.flow_limits[ll] - hp.flow_margin).max(0.0);
                    if new_flow.abs() > limit + 1e-6 {
                        feasible = false;
                        break;
                    }
                }
                ...
                for ll in 0..num_l {
                    ...
                    flows[ll] += imp_b1 * delta_b1 + imp_b2 * delta_b2;
                }
            }
        }
    }
