...
impl<'a> Evolution<'a> {
    ...
    pub fn run(
        ...
        if self.params.use_sisr {
            return self.run_sisr(rng, t0, save_solution);
        }
    ...
    fn run_sisr(
        ...
        let mut ls = LocalOps::new(self.data, self.params);
        let sisr = SisrOps::new(self.data);

        // Initial: try several Builder restarts (deterministic + randomized) and
        // pick the first that reaches feasibility after LS + repair LS.
        let mut current: Vec<Vec<usize>> = Vec::new();
        let mut current_eval: Option<Individual> = None;
        for attempt in 0..6 {
            let randomize = attempt > 0;
            let mut routes = Builder::build_routes(self.data, rng, randomize);
            ls.runls(&mut routes, rng, &self.params, false, 0);
            let ev1 = Individual::new_from_routes(self.data, &self.params, routes.clone());
            if ev1.tw_violation == 0 && ev1.load_excess == 0 {
                current = routes;
                current_eval = Some(ev1);
                break;
            }
            // Try repair pass with rising penalty multiplier.
            let factor = 100 * (attempt + 1).max(1);
            ls.runls(&mut routes, rng, &self.params, true, factor);
            let ev2 = Individual::new_from_routes(self.data, &self.params, routes.clone());
            if ev2.tw_violation == 0 && ev2.load_excess == 0 {
                current = routes;
                current_eval = Some(ev2);
                break;
            }
        }
        let mut current_eval = match current_eval {
            Some(e) => e,
            None => return None, // Truly infeasible — caller treats as no solution.
        };
        let mut best_routes = current.clone();
        let mut best_eval = current_eval.clone();
        let mut best_feasible_cost: i64 = current_eval.distance as i64;
        ...
        let total_iters = self.params.sisr_iterations.max(1);
        let t_start = self.params.sisr_t_start.max(1e-9);
        let t_end = self.params.sisr_t_end.max(1e-9);
        let cooling = (t_end / t_start).ln() / (total_iters as f64);

        let blink_rate = 0.01_f64;

        for it in 0..total_iters {
            let mut candidate = current.clone();
            let removed = sisr.ruin(
                &mut candidate,
                self.params.sisr_c_bar,
                self.params.sisr_l_max,
                self.params.sisr_alpha,
                rng,
            );
            if removed.is_empty() {
                continue;
            }
            sisr.recreate_greedy(&mut candidate, removed, rng, blink_rate);
            ls.runls(&mut candidate, rng, &self.params, false, 0);

            let mut cand_eval =
                Individual::new_from_routes(self.data, &self.params, candidate.clone());
            // If recreate or LS left a violation, try a repair pass before accepting.
            if cand_eval.tw_violation != 0 || cand_eval.load_excess != 0 {
                ls.runls(&mut candidate, rng, &self.params, true, 100);
                cand_eval = Individual::new_from_routes(self.data, &self.params, candidate.clone());
            }

            // Reject infeasible candidates outright — the SA trajectory stays on the
            // feasible manifold (standard SISR + SA-feasible scheme).
            if cand_eval.tw_violation != 0 || cand_eval.load_excess != 0 {
                continue;
            }

            let delta = cand_eval.distance as i64 - current_eval.distance as i64;
            let t = t_start * (cooling * (it as f64)).exp();
            ...
            if accept {
                ...
                let cur_dist = current_eval.distance as i64;
                if cur_dist < best_feasible_cost {
                    best_routes = current.clone();
                    best_eval = current_eval.clone();
                    best_feasible_cost = cur_dist;
                    if let Some(save) = save_solution {
                        let _ = save(&Solution {
                            routes: best_routes.clone(),
                        });
            ...
            if it % 200 == 0 {
                ...
                eprintln!(
                    "[sisr] it={} elapsed={:.1}s T={:.3} cur_dist={} best_dist={}",
                    it, elapsed, t, current_eval.distance, best_feasible_cost,
                );
            }
        }

        {
            ...
            ls.runls(&mut best_routes, rng, &self.params, false, 0);
            let final_eval =
                Individual::new_from_routes(self.data, &self.params, best_routes.clone());
            let chosen = if final_eval.tw_violation == 0
                && final_eval.load_excess == 0
                && final_eval.distance <= best_eval.distance
            {
                final_eval
            } else {
                best_eval
