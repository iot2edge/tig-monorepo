...
pub fn help() {
    println!("SAT turbo - Dual-mode WalkSAT solver");
    println!("Low density (<4.25): fast mode with perturbation escape");
    println!("High density (>=4.25): continuous run for phase transition");
}
...
pub fn solve_challenge(
    ...
    let hp: Option<Hyperparameters> = hyperparameters
        .as_ref()
...

    {
        ...
        for orig in &challenge.clauses {
            ...
            let va = (a.abs() - 1) as usize;
            if a > 0 {
                p_data[p_pos[va] as usize] = ci;
                p_pos[va] += 1;
            } else {
                n_data[n_pos[va] as usize] = ci;
                n_pos[va] += 1;
            }
            if b != a {
                let vb = (b.abs() - 1) as usize;
                if b > 0 {
                    p_data[p_pos[vb] as usize] = ci;
                    p_pos[vb] += 1;
                } else {
                    n_data[n_pos[vb] as usize] = ci;
                    n_pos[vb] += 1;
                }
            }
            if c != a && c != b {
                let vc = (c.abs() - 1) as usize;
                if c > 0 {
                    p_data[p_pos[vc] as usize] = ci;
                    p_pos[vc] += 1;
                } else {
                    n_data[n_pos[vc] as usize] = ci;
                    n_pos[vc] += 1;
                }
            }
            ci += 1;
...
fn solve_high_density(
    ...
    let max_fuel = hp
        .as_ref()
        .and_then(|h| h.max_fuel_high)
        .unwrap_or(250_000_000_000.0);
    ...
    let check_interval = (base_interval * density_factor_ci * (1.0 + (density / 3.0).ln().max(0.0)))
        ...
        loop {
                    ...
                    if stagnation >= 4 {
                        ...
                        for _ in 0..kicks {
                            ...
                            let (is, ie, ia) = if was_true {
                                (
                                    *n_off.get_unchecked(v),
                                    *n_off.get_unchecked(v + 1),
                                    &n_data,
                                )
                            } else {
                                (
                                    *p_off.get_unchecked(v),
                                    *p_off.get_unchecked(v + 1),
                                    &p_data,
                                )
                            };
            ...
            'outer_h: for j in cs..ce {
                ...
                let (os, oe, arr) = if *vars.get_unchecked(abs_l) {
                    (
                        *p_off.get_unchecked(abs_l),
                        *p_off.get_unchecked(abs_l + 1),
                        &p_data,
                    )
                } else {
                    (
                        *n_off.get_unchecked(abs_l),
                        *n_off.get_unchecked(abs_l + 1),
                        &n_data,
                    )
                };
                for k in os..oe {
            ...
            let (is, ie, ia) = if was_true {
                (
                    *n_off.get_unchecked(v_idx),
                    *n_off.get_unchecked(v_idx + 1),
                    &n_data,
                )
            } else {
                (
                    *p_off.get_unchecked(v_idx),
                    *p_off.get_unchecked(v_idx + 1),
                    &p_data,
                )
            };
            let (ds, de, da) = if was_true {
...
fn solve_low_density(
    ...
    let max_fuel = hp
        .as_ref()
        .and_then(|h| h.max_fuel_low)
        .unwrap_or(150_000_000_000.0);
    let avg_clause_size = cl.len() as f64 / nc as f64;
    let difficulty_factor = density * avg_clause_size.sqrt();
    let scale_factor = if nv > 25000 { 1.5 } else { 1.0 };
    let base_fuel = (2000.0 + 100.0 * difficulty_factor) * (nv as f64).sqrt() * scale_factor;
    ...
    let check_interval = hp.as_ref().and_then(|h| h.check_interval).unwrap_or(
    ...
    let perturbation_flips = hp
        .as_ref()
        .and_then(|h| h.perturbation_flips)
        .unwrap_or(1 + (2.0 * size_scale) as usize);
    let stagnation_limit = hp
        .as_ref()
        .and_then(|h| h.stagnation_limit)
