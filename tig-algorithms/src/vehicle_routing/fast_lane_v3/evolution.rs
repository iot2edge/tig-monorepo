...
impl<'a> Evolution<'a> {
    ...
    fn mutate_tour(&mut self, tour: &mut Vec<usize>, rng: &mut SmallRng) {
        ...
        if n >= 8 && rng.gen_ratio(1, 8) {
            ...
            let c = cuts[2];
            let d = cuts[3];
            self.mutate_scratch.clear();
            self.mutate_scratch.extend_from_slice(&tour[0..a]);
            self.mutate_scratch.extend_from_slice(&tour[c..d]);
            self.mutate_scratch.extend_from_slice(&tour[b..c]);
            self.mutate_scratch.extend_from_slice(&tour[a..b]);
            self.mutate_scratch.extend_from_slice(&tour[d..n]);
            tour.clear();
            tour.extend_from_slice(&self.mutate_scratch);
        }
    }
    ...
    pub fn split(&mut self, giant: &Vec<usize>, target_routes: usize) -> Vec<Vec<usize>> {
        ...
        let mut best_val = self.split_dp[stride + n];
        for kk in 2..=k {
