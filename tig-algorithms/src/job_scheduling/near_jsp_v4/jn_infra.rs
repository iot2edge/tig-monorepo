...
pub fn lns_disj_post(
    ...
    for _ in 0..iters {
        ...
        let machines_with_jobs: Vec<usize> = (0..ds.num_machines)
            ...
            .collect();
        if machines_with_jobs.is_empty() {
            break;
        }
            ...
            1 => {
                ...
                if let Some(ev) = eval_disj(&ds, &mut buf) {
                    if ev.0 < best_mk {
                        ...
                        ds.machine_seq[m].remove(ins);
                        ds.machine_seq[m].insert(from, node);
                    }
                } else {
                    ds.machine_seq[m].remove(ins);
                    ds.machine_seq[m].insert(from, node);
...
pub fn cp_pair_descent(
    ...
    let mut ds = build_disj_from_solution(pre, challenge, base_sol)?;
    let mut buf = EvalBuf::new(ds.n);
    let Some((mut best_mk, _)) = eval_disj(&ds, &mut buf) else {
        return Ok(None);
    };
    let initial_mk = best_mk;
    ...
    while improved && iters < max_iters {
        ...
        while curr != NONE_USIZE {
            cp.push(curr);
            curr = buf.best_pred[curr];
        }
        cp.reverse();
        ...
        for idx in 0..cp.len().saturating_sub(1) {
            ...
            let j_u = ds.node_job[u];
            let op_u = ds.node_op[u];
            let j_v = ds.node_job[v];
            let op_v = ds.node_op[v];
            ...
            if opts_u.len() < 2 && opts_v.len() < 2 {
                continue;
            }

            let orig_m_u = ds.node_machine[u];
            let orig_pt_u = ds.node_pt[u];
            let orig_m_v = ds.node_machine[v];
            let orig_pt_v = ds.node_pt[v];
            ...
            for &(m_u, pt_u) in opts_u.iter().take(4) {
                for &(m_v, pt_v) in opts_v.iter().take(4) {
                    if m_u == orig_m_u && m_v == orig_m_v {
                        continue;
                    }

                    let old_seq_mu = ds.machine_seq[orig_m_u].clone();
                    let old_seq_mv = if orig_m_u != orig_m_v {
                        ds.machine_seq[orig_m_v].clone()
                    } else {
                        vec![]
                    };

                    ds.machine_seq[orig_m_u].retain(|&x| x != u && x != v);
                    if orig_m_u != orig_m_v {
                        ds.machine_seq[orig_m_v].retain(|&x| x != u && x != v);
                    }

                    ds.node_machine[u] = m_u;
                    ds.node_pt[u] = pt_u;
                    ds.node_machine[v] = m_v;
                    ds.node_pt[v] = pt_v;

                    let len_u = ds.machine_seq[m_u].len();
                    let pos_u_opts: Vec<usize> = if len_u == 0 {
                        vec![0]
                    } else if len_u <= 2 {
                        (0..=len_u).collect()
                    } else {
                    ...
                    for &p_u in &pos_u_opts {
                        ...
                        let pos_v_opts: Vec<usize> = if len_v == 0 {
                            vec![0]
                        } else if len_v <= 2 {
                            (0..=len_v).collect()
                        } else {
                    ...
                    // Restore
                    ds.node_machine[u] = orig_m_u;
                    ds.node_pt[u] = orig_pt_u;
                    ds.node_machine[v] = orig_m_v;
                    ds.node_pt[v] = orig_pt_v;
                    ds.machine_seq[orig_m_u] = old_seq_mu;
                    if orig_m_u != orig_m_v {
                        ds.machine_seq[orig_m_v] = old_seq_mv;
                    }
                }
            }
...
pub fn schrage_pass(
    ...
    for _ in 0..max_iters {
        ...
        let mut m_order: Vec<usize> = (0..ds.num_machines)
            .filter(|&m| mc[m] > 0 && ds.machine_seq[m].len() >= 2)
            .collect();
...
pub fn slack_ejection_schrage(
    ...
    let mut ds = build_disj_from_solution(pre, challenge, base_sol)?;
    let mut buf = EvalBuf::new(ds.n);
    let Some((mut cur_mk, mut terminal)) = eval_disj(&ds, &mut buf) else {
        return Ok(None);
    };
    let init_mk = cur_mk;
    for _ in 0..max_iters {
        ...
        let cp_flex: Vec<usize> = cp
            .iter()
            .filter(|&&nd| {
                if ds.node_job[nd] >= pre.job_products.len() {
                    return false;
                }
                let prod = pre.job_products[ds.node_job[nd]];
                let op = ds.node_op[nd];
                prod < pre.product_ops.len()
                    && op < pre.product_ops[prod].len()
        ...
        'node_loop2: for &nd in &cp_flex {
            ...
            let orig_m = ds.node_machine[nd];
            let orig_pt = ds.node_pt[nd];
            if job >= pre.job_products.len() {
                continue;
            }
            let prod = pre.job_products[job];
            if op_idx >= pre.product_ops[prod].len() {
                continue;
            }
                    ...
                    if let Some((cand_mk, _)) = eval_disj(&ds, &mut buf) {
                        if cand_mk < best_mk2 {
                            ...
                            best_commit = Some((
                                new_m,
                                new_pt,
                                ds.machine_seq[orig_m].clone(),
                                ds.machine_seq[new_m].clone(),
                            ));
                        }
                    }
                }
...
pub fn bottleneck_machine_relief_pass(
    ...
    for iter in 0..max_iters {
        ...
        if best_nd != usize::MAX && best_iter_mk < current_mk {
            ...
            let old_pos = ds.machine_seq[cur_m]
                .iter()
                .position(|&x| x == best_nd)
                .unwrap();
            ds.machine_seq[cur_m].remove(old_pos);
            ...
            machine_loads[best_new_m] =
                machine_loads[best_new_m].saturating_add(best_new_pt as u64);
