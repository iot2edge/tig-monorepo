    ...
    for _iter in 0..max_iters {
        ...
        for cand in &cands {
            if cand.kind == 0 {
                ...
                if let Some((node2, old_pt, ins_idx)) =
                    apply_reroute(ds, m_from, cand.from, m_to, cand.to, cand.new_pt)
                {
                    if let Some((mk2, _)) = eval_disj(ds, buf) {
                        if mk2 < best_mk {
                            best_mk = mk2;
        ...
        if bc.kind == 0 {
            ...
            if let Some(ne) = eval_disj(ds, buf) {
                if ne.0 < cur_mk {
                    *cur_eval = ne;
                    cur_mk = ne.0;
                    improved = true;
            ...
            if let Some((node2, old_pt, ins_idx)) =
                apply_reroute(ds, bc.m_from, bc.from, bc.m_to, bc.to, bc.new_pt)
            {
                if let Some(ne) = eval_disj(ds, buf) {
                    if ne.0 < cur_mk {
                        *cur_eval = ne;
...
pub fn apply_reroute(
    ...
    if m_from >= ds.num_machines
        || m_to >= ds.num_machines
        || idx_from >= ds.machine_seq[m_from].len()
    {
        return None;
    }
...
pub fn undo_reroute(
    ...
    if m_from >= ds.num_machines || m_to >= ds.num_machines || ins_idx >= ds.machine_seq[m_to].len()
    {
        return false;
    }
    ...
    if x != node {
        ...
        ds.machine_seq[m_to].insert(ins_idx.min(len_now), x);
        return false;
    }
...
fn build_perm_solution_from_seq_local(
    seq: &[usize],
    route: &[usize],
    pt: &[Vec<u32>],
    num_jobs: usize,
