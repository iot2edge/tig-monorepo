...
pub fn solve(
                        ...
                        for b in 0..np {
                            ...
                            for c in 0..np {
                                ...
                                'outer: for i in 0..sl_ab {
                                    ...
                                    if $partition_mut_swap[node_ab] as usize != a {
                                        continue;
                                    }
                                    for j in 0..sl_bc {
                                        let (node_bc, gain_bc) = $part_to_part[idx_bc][j];
                                        if $partition_mut_swap[node_bc] as usize != b {
                                            continue;
                                        }
                                        if node_bc == node_ab {
                                            continue;
                                        }
                                        if gain_ab + gain_bc <= 0 {
                                            break;
                                        }
                                        for k in 0..sl_ca {
                                            let (node_ca, gain_ca) = $part_to_part[idx_ca][k];
                                            if $partition_mut_swap[node_ca] as usize != c {
                                                continue;
                                            }
                                            if node_ca == node_ab || node_ca == node_bc {
                                                continue;
                                            }
    ...
    let mut conn_with_idx: Vec<(i32, i32)> = conn_vec
        .iter()
        .enumerate()
        .map(|(i, &c)| (c, i as i32))
        .collect();
