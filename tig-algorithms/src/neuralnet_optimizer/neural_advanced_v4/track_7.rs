...
fn optimizer_init(
    ...
    for &n in param_sizes {
        ...
        if n > 0 {
            stream.memcpy_htod(&host_fisher[..n], &mut fisher_init)?;
        }
        f.push(fisher_init);
...
fn optimizer_step(
    ...
    let trend_factor: f32 = if s.val_loss_history.len() >= 3 {
        ...
        if recent_slope > 0.015 {
            0.88
        } else if recent_slope > 0.008 {
            0.94
        } else {
            1.0
        }
    ...
    if do_svrg {
        ...
        for i in 0..num_layers {
            ...
            let g = &gradients[i];
            let ef_ref = &s.ef[i];
            if let (Some(vr_buf), Some(mean_buf)) =
                (vr_grad_bufs[i].as_mut(), ef_mean_bufs[i].as_mut())
            {
                unsafe {
                    stream
                        .launch_builder(&k_svrg)
                        .arg(g)
                        .arg(ef_ref)
                        .arg(mean_buf)
                        .arg(vr_buf)
    ...
    for (i, g) in gradients.iter().enumerate() {
        ...
        let effective_lr =
            lr * layer_multiplier * precision_gain * forward_gain * trust_backoff * trend_factor;
        let cfg = s.cfgs[i];
    ...
    if do_adaptive_lr {
        ...
        if prev_norm_sum > 1e-8 {
            for i in 0..num_layers {
                ...
                let layer_share = if curr_norm_sum > 1e-8 {
                    curr / (curr_norm_sum + 1e-8)
                } else {
                    1.0 / num_layers as f32
                };
                let prev_est = prev_norm_sum * layer_share;
                let ratio = curr / (prev_est + 1e-8);
                let initial_lr = if i == num_layers - 1 {
                    0.00065f32
                } else if i == 0 {
                    0.00110f32
                } else if gradients[i].len() <= 256 {
                    0.00240f32
                } else if gradients[i].len() <= 512 {
                    0.00200f32
                } else if gradients[i].len() > 50000 {
                    0.00095f32
                } else {
                    0.00130f32
                };
                let scale = if ratio > 1.5 {
                    0.9f32
                } else if ratio < 0.5 {
                    1.1f32
                } else {
