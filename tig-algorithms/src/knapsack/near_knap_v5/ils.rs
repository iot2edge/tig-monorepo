...
pub fn run_one_instance(
    challenge: &Challenge,
    params: &Params,
    save_solution: Option<&dyn Fn(&Solution) -> anyhow::Result<()>>,
) -> Solution {
    let checkpoint = |best_sel: &[usize]| {
        if let Some(save) = save_solution {
            let _ = save(&Solution {
                items: best_sel.to_vec(),
            });
        }
    };
    ...
    if n >= 2000 {
        effective_params.extra_starts = params.extra_starts + 1;
        if n < 3500 {
            effective_params.dp_passes_multiplier = params.dp_passes_multiplier * 2;
            effective_params.n_perturbation_rounds = params.n_perturbation_rounds + 5;
        }
    } else {
        effective_params.n_perturbation_rounds = params.n_perturbation_rounds + 15;
    }
    let params = &effective_params;

    n_starts += params.extra_starts;

    let n_starts_extended = if n >= 4500 {
        n_starts
    } else if n >= 2000 {
        n_starts.max(if n >= 4000 { 4 } else { 5 })
    } else {
        n_starts
    };
    let mut population: Vec<State> = Vec::with_capacity(n_starts_extended + 8);

    for sid in 0..n_starts_extended {
        ...
        match sid {
            ...
            3 => {
                construct_forward_incremental(&mut st, if hard { 5 } else { 4 }, &mut rng);
                rebuild_windows(&mut st);
            }
            _ => {
                let m = (sid as usize) % 6 + 2;
        ...
        let scratch: Vec<usize> = (0..n).filter(|&i| st.selected_bit[i]).collect();
        checkpoint(&scratch);
        population.push(st);
    }

    population.sort_unstable_by(|a, b| b.total_value.cmp(&a.total_value));
    let pop_size_target: usize = 6;
    if population.len() > pop_size_target {
        population.truncate(pop_size_target);
    }

    if let Some(top) = population.first() {
        let scratch: Vec<usize> = (0..n).filter(|&i| top.selected_bit[i]).collect();
        checkpoint(&scratch);
    }

    let n_crossover_gens = if n < 2000 || n >= 4500 {
        0
    } else if n >= 4000 {
        2
    } else {
    ...
    for _gen in 0..n_crossover_gens {
        if population.len() < 2 {
            break;
        }

        // Frequency-based crossover from full population
        let pop_size = population.len();
        let mut freq = vec![0usize; n];
        for sol in population.iter() {
            for i in 0..n {
                if sol.selected_bit[i] {
                    freq[i] += 1;
                }
            }
        }
        let threshold = (pop_size * 3) / 4;
        let mut child = State::new_empty(challenge, &total_pre, &hubs_static, neigh_pre.as_ref());
        ...
        for &i in &consensus {
            if child.total_weight + challenge.weights[i] <= challenge.max_weight {
                child.add_item(i);
            }
        }
        for &i in &exploratory {
            if rng.next_u32() % 2 == 0
                && child.total_weight + challenge.weights[i] <= challenge.max_weight
            {
                child.add_item(i);
            }
        }
        rebuild_windows(&mut child);
        dp_refinement_x(&mut child, params.dp_passes_multiplier);
        rebuild_windows(&mut child);
        micro_qkp_refinement(&mut child);
        local_search_vnd(&mut child, params);
        population.push(child);

        // Uniform pairwise crossover between top two
        if population.len() >= 2 {
            let a_bits = population[0].selected_bit.clone();
            let b_bits = population[1].selected_bit.clone();
            let mut child2 =
                State::new_empty(challenge, &total_pre, &hubs_static, neigh_pre.as_ref());
            for i in 0..n {
                if a_bits[i]
                    && b_bits[i]
                    && child2.total_weight + challenge.weights[i] <= challenge.max_weight
                {
                    child2.add_item(i);
                }
            }
            for i in 0..n {
                if child2.selected_bit[i] {
                    continue;
                }
            ...
            rebuild_windows(&mut child2);
            dp_refinement_x(&mut child2, params.dp_passes_multiplier);
            rebuild_windows(&mut child2);
            micro_qkp_refinement(&mut child2);
            local_search_vnd(&mut child2, params);
    ...
    let mut best: Option<State> = if !population.is_empty() {
        Some(population.remove(0))
    } else {
        None
    };
    let mut second: Option<State> = if !population.is_empty() {
        Some(population.remove(0))
    } else {
        None
    };

    if n < 2000 && best.is_some() && second.is_some() {
    ...
    let kick_fuel_threshold: u64 = if n >= 4000 {
        1_800_000_000
    } else if n >= 2000 {
        700_000_000
    } else {
    ...
    for relink_pass in 0..3 {
        if fuel_remaining() < kick_fuel_threshold {
            break;
        }
        ...
        if relink_pass == 0
            && basins_close
            && int_items_orig != best_sel
            && fuel_remaining() >= kick_fuel_threshold
        {
    ...
    let kick_sizes: &[usize] = if n >= 4500 {
        &[5, 10, 18, 28, 40, 55, 75, 8, 22, 50, 14, 33, 65, 12]
    } else if n >= 4000 {
        &[
            5, 10, 18, 28, 40, 55, 75, 8, 22, 50, 14, 33, 65, 12, 38, 90, 7, 25, 47, 80, 16, 30,
            58, 105, 20,
        ]
    } else if n >= 2000 {
        &[
            3, 6, 11, 18, 28, 40, 55, 4, 14, 35, 9, 23, 48, 7, 30, 65, 5, 17, 38, 60, 12, 21, 45,
            75, 15, 26, 52, 80, 19, 70, 33, 90, 24, 58, 100, 8, 16, 27, 42, 62, 85, 10, 36, 50, 70,
            13, 22, 31, 55, 78,
        ]
    } else {
        &[
            2, 4, 7, 11, 16, 22, 30, 3, 9, 20, 6, 14, 26, 5, 18, 35, 4, 12, 24, 40, 8, 13, 21, 32,
            10, 17, 28, 45, 6, 38, 15, 50, 11, 33, 60, 7, 19, 29, 42, 55, 9, 23, 36, 48, 14, 25,
            31, 38, 52, 65,
        ]
    };
    let kick_passes = if n >= 4000 { 1 } else { 2 };
    'kick_outer: for _kick_pass in 0..kick_passes {
        ...
        for &k_kick in kick_sizes {
            ...
            if adopt(&mut best_sel, &mut best_val, &kicked_polish) {
                pass_improved = true;
            }
        }
        if !pass_improved {
    ...
    if fuel_remaining() >= kick_fuel_threshold {
        let mut contrib_state =
            State::new_empty(challenge, &total_pre, &hubs_static, neigh_pre.as_ref());
        for &i in &best_sel {
            contrib_state.add_item(i);
        }
        let mut by_contrib: Vec<(i32, usize)> = best_sel
            .iter()
            .map(|&i| (contrib_state.contrib[i], i))
            .collect();
        by_contrib.sort_unstable_by_key(|&(c, _)| c);

        let contrib_kick_sizes: &[usize] = if n >= 4500 {
            &[8, 16, 25]
        } else if n >= 4000 {
            &[8, 16, 25, 40, 60]
        } else if n >= 2000 {
        ...
        for &k_kick in contrib_kick_sizes {
            if fuel_remaining() < kick_fuel_threshold {
                break;
            }
            if by_contrib.len() <= k_kick + 1 {
                continue;
            }
            let kept_items: Vec<usize> = by_contrib[k_kick..].iter().map(|&(_, i)| i).collect();
            let kicked_polish = polish_from(&kept_items, cap);
