...
mod dead_code {
    ...
    pub fn solve_challenge(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
        ...
        let vector_cost = if challenge.num_queries < 240 {
            1680.0
        } else if challenge.num_queries < 450 {
            1700.0
        } else if challenge.num_queries > 1250 {
            1740.0
        } else {
            1720.0
        };
        let alpha = vector_cost * challenge.num_queries as f64;
        ...
        let closest_vectors =
            filter_relevant_vectors(&challenge.vector_database, &challenge.query_vectors, n);

        let (m_slice, r_slice) = closest_vectors.split_at(m);
        let m_vectors: Vec<_> = m_slice.to_vec();
        let r_vectors: Vec<_> = r_slice.to_vec();

        let mut kd_tree_vectors: Vec<(&[f32], usize)> =
            m_vectors.iter().map(|&(_, v, i)| (v, i)).collect();
        ...
        let high_distance_indices: Vec<_> = distance_indices
            .into_iter()
