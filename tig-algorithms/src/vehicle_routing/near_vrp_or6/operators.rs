...
impl<'a> LocalOps<'a> {
    ...
    pub fn run_intra_route_oropt(&mut self, r1: usize, pos1: usize, l: usize) -> bool {
        // Generalized or-opt: relocate a contiguous block of l customers to
        // any other position in the same route. The original fast_lane_v4
        // hardcoded l=2,3; this version accepts any l >= 2 (block_seq is
        // built by chaining seq1 segments).
        if l < 2 {
            return false;
        }
        let old_cost = self.routes[r1].cost;
        ...
        let applied = {
            ...
            // The block spans inner positions [pos1..pos1+l). Need the depot
            // (or another customer) at pos1+l, so pos1+l <= len-1 i.e. < len.
            if pos1 + l >= len {
                return false;
            }

            // Build block_seq = join of seq1[pos1], seq1[pos1+1], ..., seq1[pos1+l-1].
            let mut block_seq = route.nodes[pos1].seq1;
            for offset in 1..l {
                block_seq =
                    RouteEval::join2(self.data, &block_seq, &route.nodes[pos1 + offset].seq1);
            }
        ...
        while improved {
            ...
            for &c1 in &c1_order {
                ...
                if self.when_last_modified[r1] > last_tested {
                    ...
                    // Extended block moves (l=4,5,6) — these are k-opt block
                    // relocations not present in fast_lane_v4. Conditioned on
                    // a config flag so we can ablate.
                    if self.params.use_or6 {
                        improved |=
                            self.run_intra_route_oropt(self.node_route[c1], self.node_pos[c1], 4);
                        improved |=
                            self.run_intra_route_oropt(self.node_route[c1], self.node_pos[c1], 5);
                        improved |=
                            self.run_intra_route_oropt(self.node_route[c1], self.node_pos[c1], 6);
                    }
                }
            }
        }
