...
pub fn solve_challenge(
    ...
    if density >= 4.25 {
        if nv <= 5000 {
            return solve_high_density_sparse(
                &hp,
                &mut rng,
                nv,
                nc,
...
fn solve_high_density(
    ...
    let default_fuel = 5_000_000_000_000.0;
    let max_fuel = hp
        .as_ref()
        ...
        loop {
                    ...
                    if stagnation >= 4 {
                        ...
                        for _ in 0..kicks {
                            ...
                            let (is, ie, ia) = if was_true {
                                (
                                    *n_off.get_unchecked(v) as usize,
                                    *n_off.get_unchecked(v + 1) as usize,
                                    n_data,
                                )
                            } else {
                                (
                                    *p_off.get_unchecked(v) as usize,
                                    *p_off.get_unchecked(v + 1) as usize,
                                    p_data,
                                )
                            };
            ...
            'outer_h: for j in cs..ce {
                ...
                let (os, oe) = if *vars.get_unchecked(abs_l) {
                    (
                        *p_off.get_unchecked(abs_l) as usize,
                        *p_off.get_unchecked(abs_l + 1) as usize,
                    )
                } else {
                    (
                        *n_off.get_unchecked(abs_l) as usize,
                        *n_off.get_unchecked(abs_l + 1) as usize,
                    )
                };
                let arr: &[u32] = if *vars.get_unchecked(abs_l) {
                    p_data
                } else {
                    n_data
                };
