...
fn tabu_search_fjsp_medium(
        ...
        if iter % 3 == 0 {
            for node in 0..n {
                ...
                for &(new_m, new_pt) in &op_info.machines {
                    ...
                    let positions =
                        find_insert_positions_fm(&ds, &buf.start, node, new_m, &job_pred_node);
                    for insert_pos in positions {
                        ...
                        if (!is_tabu || aspiration) && est_mk < best_move_mk {
                            best_move_mk = est_mk;
                            best_move = Some(MoveTypeFm::Reassign {
                                node,
                                new_machine: new_m,
                                new_pt,
                                insert_pos,
                        ...
                        if est_mk < fallback_mk {
                            fallback_mk = est_mk;
                            fallback_move = Some(MoveTypeFm::Reassign {
                                node,
                                new_machine: new_m,
                                new_pt,
                                insert_pos,
    ...
    let Some((_, _)) = eval_disj(&best_global_ds, &mut buf) else {
        return Ok(None);
    };
...
pub fn solve(
    ...
    for r in 0..num_restarts {
        ...
        let target = if best_mk < u32::MAX / 2 {
            Some(best_mk.saturating_add(target_margin))
        } else {
            None
        };
    ...
    let route_w_ls = if route_w_base > 0.0 {
    ...
    for i in 0..refine_n {
        ...
        let target_ls = if best_mk < u32::MAX / 2 {
            Some(best_mk.saturating_add(target_margin / 2))
        } else {
            None
        };
        for attempt in 0..8 {
    ...
    if let Ok(Some((bmr_sol, bmr_mk))) =
        bottleneck_machine_relief_pass(pre, challenge, &top_solutions[0].0, 20)
    {
        if bmr_mk < best_mk {
            best_mk = bmr_mk;
    ...
    for ils_r in 0..ils_rounds {
        ...
        let n = ds.n;
        let mut pseed: u64 = (ils_r as u64)
        ...
        if strategy == 0 {
            ...
            while perturbed < k_perturb && attempts < crit_nodes.len() * 4 {
                ...
                let old_pos = match ds.machine_seq[cur_m].iter().position(|&x| x == node) {
                    Some(p) => p,
                    None => continue,
                };
            ...
            let mut swapped = 0;
            for _ in 0..(k_sw * 4) {
                if swapped >= k_sw || crit_machines.is_empty() {
                    break;
                }
                pseed ^= pseed.wrapping_shl(13);
                pseed ^= pseed.wrapping_shr(7);
            ...
            for &nd in &crit_nodes {
                machine_crit_count[ds.node_machine[nd]] += 1;
            }
            let target_m = machine_crit_count
                .iter()
                .enumerate()
                .max_by_key(|&(_, &c)| c)
                .map(|(m, _)| m)
                .unwrap_or(0);
            let candidates: Vec<usize> = crit_nodes
                .iter()
                .cloned()
                .filter(|&nd| {
                    if ds.node_machine[nd] != target_m {
                        return false;
                    }
        ...
        let (candidate_sol, candidate_mk) = match critical_block_move_local_search_ex(
            pre,
            challenge,
            &after_gr.0,
            cb_passes,
        ...
        if candidate_mk < ils_best_mk {
            ...
            if candidate_mk < best_mk {
                best_mk = candidate_mk;
    ...
    // Calibrated: 310k / ts_iters ≈ 192 outer iters → cycle ≈ 615s at 5T
    {
        ...
        for loop_iter in 0..max_outer_iters {
            ...
            let perturbed = match targeted_cp_kick(
                pre,
                challenge,
                &base_sol,
                kick_reassigns,
            ...
            let (cand_sol, cand_mk) = match tabu_search_fjsp_medium(
                ...
                None => {
                    ...
                    match critical_block_move_local_search_ex(
                        pre,
                        challenge,
                        &perturbed,
                        cb_passes,
