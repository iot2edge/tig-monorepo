...
// (job_nine beats both our solver and job_eight on this scenario).
...
pub fn solve_challenge(
    ...
    let optimizer_result: Result<()> = match detected {
        ...
        DetectedTrack::Complex => {
            // ≈ FJSP_MEDIUM — single-engine. Fuel measurement showed our
            // solver wants ~48B but the production budget is 10B; job_nine's
            // wants ~85B. Neither has spare fuel, so a dual-solver split is
            // impossible — running both just starves both. Our solver wins
            // 13/20 nonces because it degrades more gracefully under the cap.
            //   * Low fuel (< 30B): v4's fjsp_medium (faster).
            //   * High fuel (≥ 30B): v3's fjsp_medium.
            if fuel_remaining() < HIGH_FUEL_THRESHOLD {
                let pre = v4_build_pre(challenge)?;
                // fjsp_medium_iters defaults to 200. A 2026-05-14 fuel sweep
                // at the production 10B budget found 200 is the peak (2-seed
                // avg 57,229) — beats the prior default of 500 by +12,249
                // (+27%). The restart loop at 500 over-spent fuel that the
                // refinement phases (tabu / critical-block / ILS / ALNS /
                // crossover) use far more productively. Plateau is flat from
                // 200-250. A `fjsp_medium_iters` hyperparameter overrides it.
