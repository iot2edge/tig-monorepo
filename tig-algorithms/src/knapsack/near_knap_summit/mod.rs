...
// kqo_v5 source as a sub-module — used as a third pass in solve_challenge
// to win n=1000,b=25 where v6 source can't beat v5 even with v5 HP.
mod quadratic_core;
mod strategic_oscillation;
mod v5_engine;
...

        {
            ...
            for si in 0..ks {
                ...
                for ai in 0..ku {
                    ...
                    let ca1_eff =
                        state.contrib[a1] as i64 - state.ch.interaction_values[a1][rm] as i64;
                    for bi in (ai + 1)..ku {
                        ...
                        let ca2_eff =
                            state.contrib[a2] as i64 - state.ch.interaction_values[a2][rm] as i64;
                        let syn = state.ch.interaction_values[a1][a2] as i64;
                        ...
                        for &a2 in &tsn.friends[a1] {
                            ...
                            if wa1 + wa2 <= budget {
                                let gained = ca1_eff + state.contrib[a2] as i64
                                    ...
                                    + state.ch.interaction_values[a1][a2] as i64;
                                if gained - lost > bd
                                    && state.total_weight + wa1 + wa2 <= cap + wr1 + wr2
                                {
                                    bd = gained - lost;
                                    bm = Some((r1, r2, a1, a2));
                                }
...
struct Hparams {
    ...
    window_k: usize,
    core_half_dp: usize,
    restart_base: usize, // External RNG offset — different basin entry points across passes
}

impl Hparams {
    // ZENITH HP: keeps v6's seeded-restart code (in Solver::solve), but uses
    // v5's deeper iter counts. Per the comparator, v5 wins 3/5 tracks despite
    // older code — its higher iters were the key. Combining v5 iters + v6
    // structural improvements should beat both.
    fn for_size(n: usize, budget: u32) -> Self {
        if n <= 1200 {
            if budget <= 5 {
                ...
                // budget>10 (i.e., b=25): bump ils_rounds further (200→300) and
                // restarts (12→16) — kqo_v5 currently wins this track by ~200.
                // We have huge fuel headroom (5T cap, using <30B).
                Self {
                    n_random_starts: 5,
                    n_crossover_gen: 16,
                    sa_rounds: 0,
    ...
    fn from_map(h: &Option<Map<String, Value>>, n: usize, budget: u32) -> Self {
        ...
        if let Some(m) = h {
            ...
            if let Some(v) = m.get("restart_base").and_then(|v| v.as_u64()) {
                p.restart_base = v as usize;
            }
        }
        p
    }
...
fn run_one_instance_seeded(
    ...
    for variant in 0..n_greedy {
        ...
        match variant {
            0 => {
                if n > 1200 {
                    build_greedy_density_from_all(&mut st, total_interactions)
                } else {
    ...
    let max_stall = if n > 1200 {
        (hp.ils_rounds / 3).min(8)
    } else {
        (hp.ils_rounds * 2 / 3).min(15)
    };
...
fn eval_solution(ch: &Challenge, sol: &Solution) -> i64 {
    ...
    for &i in &sol.items {
        ...
        for &j in &sol.items {
            if j > i {
                val += ch.interaction_values[i][j] as i64;
            }
        }
    }
    val
    ...
    pub fn solve(
        ...
        if n > 1200 {
            ...
            for restart in 0..n_restarts {
                let actual = hp.restart_base + restart; // RNG offset (different basin entry per pass)
                let (sol, val) = if restart >= 2 && restart % 2 == 0 && best_sol.is_some() {
                    ...
                    run_one_instance(challenge, &hp, actual, Some(&tsn), &total_interactions)
                };
                if val > best_quality {
                    best_quality = val;
                    best_sol = Some(sol);
        ...
        let n_phase1 = n_restarts.min(12).max(2); // Lifted cap from 6 to 12 — more depth for n<=1200 cases.
        let mut restart_stall_1k = 0usize;
        for restart in 0..n_phase1 {
            let actual = hp.restart_base + restart;
            let (sol, val) = run_one_instance(challenge, &hp, actual, None, &total_interactions);
        ...
        let n_phase3 = 6; // Bumped from 4 to 6 — more crossover-derived restarts for n<=1200 cases.
...
// HP profile variants to try. We pick the better solution of {v6-style HP, v5-style HP}
// per nonce. This is the algorithmic breakthrough: rather than committing to one
// HP set, we exploit fuel headroom (5T cap, we use <40B per profile) by running
// BOTH and keeping the best by raw quality.
fn build_hp_v6_style(n: usize, budget: u32) -> Hparams {
    if n <= 1200 {
        if budget <= 5 {
            Hparams {
                n_random_starts: 3,
...
pub fn solve_challenge(
    ...
    // PASS A: zenith default HP (v5-style: deeper iters).
    let sol_a = Solver::solve(challenge, Some(save_solution), hyperparameters)?;

    // PASS B: v6-style HP (lower iters, faster — wins on b=5 tracks per
    // comparator data). Override the defaults explicitly.
    let mut hp_b_overrides: Map<String, Value> = Map::new();
    let n = challenge.num_items;
    let sum_w: u64 = challenge.weights.iter().map(|&w| w as u64).sum();
    let budget_pct = if sum_w > 0 {
        ((challenge.max_weight as u64) * 100 / sum_w) as u32
    ...
    let hp_b = build_hp_v6_style(n, budget_pct);
    hp_b_overrides.insert(
        "n_random_starts".into(),
        Value::from(hp_b.n_random_starts as u64),
    );
    hp_b_overrides.insert(
        "n_crossover_gen".into(),
        Value::from(hp_b.n_crossover_gen as u64),
    );
    hp_b_overrides.insert("ils_rounds".into(), Value::from(hp_b.ils_rounds as u64));
    hp_b_overrides.insert(
        "ils_restart_interval".into(),
        Value::from(hp_b.ils_restart_interval as u64),
    );
    hp_b_overrides.insert(
        "n_full_restarts".into(),
        Value::from(hp_b.n_full_restarts as u64),
    );
    hp_b_overrides.insert("window_k".into(), Value::from(hp_b.window_k as u64));
    hp_b_overrides.insert("core_half_dp".into(), Value::from(hp_b.core_half_dp as u64));
    let hp_b_for_solve = Some(hp_b_overrides.clone());
    let sol_b = Solver::solve(challenge, Some(save_solution), &hp_b_for_solve)?;

    // PASS C: v5 engine source verbatim. v5's defaults already have higher
    // iters than v6, and overriding even higher hurt b=25 — leave at default.
    let sol_c = v5_engine::Solver::solve(challenge, Some(save_solution), hyperparameters)?;

    // PASS D: v6 source with WIDE HP + restart_base=100 (different RNG path).
    let mut hp_d_overrides: Map<String, Value> = Map::new();
    hp_d_overrides.insert("n_random_starts".into(), Value::from(6u64));
    hp_d_overrides.insert("n_crossover_gen".into(), Value::from(16u64));
    hp_d_overrides.insert("window_k".into(), Value::from(400u64));
    hp_d_overrides.insert("core_half_dp".into(), Value::from(80u64));
    hp_d_overrides.insert(
        "n_full_restarts".into(),
        Value::from(if n > 1200 { 30u64 } else { 14 }),
    );
    hp_d_overrides.insert("restart_base".into(), Value::from(100u64));
    let hp_d_for_solve = Some(hp_d_overrides);
    let sol_d = Solver::solve(challenge, Some(save_solution), &hp_d_for_solve)?;

    // PASS E: v5 source extreme depth + restart_base=200 (different RNG path).
    let mut hp_e_overrides: Map<String, Value> = Map::new();
    let v5_ils_e = if budget_pct <= 5 {
        250u64
    } else if budget_pct <= 10 {
        400
    } else {
        500
    };
    hp_e_overrides.insert("ils_rounds".into(), Value::from(v5_ils_e));
    hp_e_overrides.insert(
        "n_full_restarts".into(),
        Value::from(if n > 1200 { 35u64 } else { 18 }),
    );
    hp_e_overrides.insert("window_k".into(), Value::from(400u64));
    hp_e_overrides.insert("core_half_dp".into(), Value::from(80u64));
    hp_e_overrides.insert("restart_base".into(), Value::from(200u64));
    let hp_e_for_solve = Some(hp_e_overrides);
    let sol_e = v5_engine::Solver::solve(challenge, Some(save_solution), &hp_e_for_solve)?;

    // PASS F: v6 with restart_base=300 (yet another RNG path, default HP).
    let mut hp_f_overrides: Map<String, Value> = Map::new();
    hp_f_overrides.insert("restart_base".into(), Value::from(300u64));
    let hp_f_for_solve = Some(hp_f_overrides);
    let sol_f = Solver::solve(challenge, Some(save_solution), &hp_f_for_solve)?;

    // PASS G: v5 source with restart_base=400 (yet another RNG path, default HP).
    let mut hp_g_overrides: Map<String, Value> = Map::new();
    hp_g_overrides.insert("restart_base".into(), Value::from(400u64));
    let hp_g_for_solve = Some(hp_g_overrides);
    let sol_g = v5_engine::Solver::solve(challenge, Some(save_solution), &hp_g_for_solve)?;

    // ALGORITHMIC ADDITION: path-relink between pass A and pass B as a fourth
    // candidate. v6 has path_relink internally for its own population, but
    // here we relink ACROSS independent populations of two different HP runs.
    // Pick best of {a, b, c, relinked}.
    let mut candidates: Vec<(Solution, i64)> = Vec::new();
    if let Some(a) = sol_a {
        let va = eval_solution(challenge, &a);
        candidates.push((a, va));
    }
    if let Some(b) = sol_b {
        let vb = eval_solution(challenge, &b);
        candidates.push((b, vb));
    }
    if let Some(c) = sol_c {
        let vc = eval_solution(challenge, &c);
        candidates.push((c, vc));
    }
    if let Some(d) = sol_d {
        let vd = eval_solution(challenge, &d);
        candidates.push((d, vd));
    }
    if let Some(e) = sol_e {
        let ve = eval_solution(challenge, &e);
        candidates.push((e, ve));
    }
    if let Some(f) = sol_f {
        let vf = eval_solution(challenge, &f);
        candidates.push((f, vf));
    }
    if let Some(g) = sol_g {
        let vg = eval_solution(challenge, &g);
        candidates.push((g, vg));
    }
    if candidates.is_empty() {
        return Ok(());
    }

    // STRUCTURAL ADDITION: strategic oscillation from the best ILS candidate.
    // Strategic oscillation deliberately drifts to INFEASIBILITY (alpha-overload
    // on weight) then removes items to drift back, using tabu memory to avoid
    // cycling. This explores corners of the feasible region that ILS
    // perturbations cannot — wholesale replacement of items, not small flips.
    //
    // Cost: ~n^2 per cycle × max_cycles × 6 configs. For n=1000, max_cycles=30,
    // that's ~1.8B ops per nonce. Within fuel budget. For n=5000, scaled down.
    let osc_max_cycles: u32 = if n <= 1200 { 40 } else { 20 };
    {
        // Pick best feasible candidate as seed for oscillation.
        let (best_idx, _) = candidates
            .iter()
            .enumerate()
            .max_by_key(|(_, (_, v))| *v)
            .unwrap();
        let seed_sol = candidates[best_idx].0.clone();
        let seed_val = candidates[best_idx].1;
        let (osc_sol, osc_val) =
            strategic_oscillation::oscillate_multi(challenge, &seed_sol, seed_val, osc_max_cycles);
        if osc_val > seed_val {
            candidates.push((osc_sol, osc_val));
        } else {
            // Even ties enter the pool — path-relink with this distinct basin
            // may yield improvement.
            candidates.push((osc_sol, osc_val));
        }
    }

    // Pairwise path-relink across ALL pairs (a-b, a-c, b-c). Each relink
    // explores intermediate solutions on the path between two basins.
    let hp_relink = Hparams::from_map(hyperparameters, n, budget_pct);
    let initial_n = candidates.len();
    for i in 0..initial_n {
        for j in (i + 1)..initial_n {
            let relinked = path_relink(challenge, &candidates[i].0, &candidates[j].0, &hp_relink);
            let vr = eval_solution(challenge, &relinked);
            candidates.push((relinked, vr));
        }
    }

    // Pick max-quality candidate.
    candidates.sort_by_key(|(_, v)| std::cmp::Reverse(*v));
    let (pick, pick_val) = candidates.into_iter().next().unwrap();

    // STRUCTURAL INNOVATION: exact quadratic core refinement.
    //
    // Up to here, every solver in the chain (and zenith) uses linear DP on a
    // core of items, ignoring core-core interactions. This pass enumerates 2^K
    // subsets of a K-item core via Gray code, computing the true quadratic
    // objective for each. This is the first refinement that accounts for
    // joint moves where two core items reinforce each other.
    //
    // K is adapted to n: larger instances → smaller K (2^K cost is constant in
    // n, but per-iter pair-eval grows with fixed_sel size).
    let k_core: usize = if n <= 1200 { 22 } else { 20 };
    let (refined, refined_val) = quadratic_core::refine_iterative(challenge, &pick, k_core, 3);
    let final_sol = if refined_val > pick_val {
        refined
    } else {
        pick
    };
    let _ = save_solution(&final_sol);
    Ok(())
}

pub fn help() {
    println!("knap_quality_opt_v6");
}
