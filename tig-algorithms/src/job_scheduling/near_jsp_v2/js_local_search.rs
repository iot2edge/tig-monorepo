...
/// Tabu key for a candidate move — uses node ids (not positions) so a swap that
/// gets undone by a later swap of the same two nodes hits the tabu list.
fn move_tabu_key(ds: &DisjSchedule, c: &MoveCand) -> u64 {
    match c.kind {
        2 => {
            let m = c.m_from;
            if m < ds.num_machines && c.from + 1 < ds.machine_seq[m].len() {
                let a = ds.machine_seq[m][c.from] as u64;
                let b = ds.machine_seq[m][c.from + 1] as u64;
                let (lo, hi) = if a < b { (a, b) } else { (b, a) };
                (2u64 << 60) | ((lo & 0xFFFF_FFFF) << 28) | (hi & 0x0FFF_FFFF)
            } else {
                u64::MAX
            }
        }
        0 => {
            let m = c.m_from;
            let node = if m < ds.num_machines && c.from < ds.machine_seq[m].len() {
                ds.machine_seq[m][c.from] as u64
            } else {
                0
            };
            (0u64 << 60) | ((node & 0xFFFF_FFFF) << 28) | ((c.to as u64) & 0x0FFF_FFFF)
        }
        1 => {
            let m = c.m_from;
            let node = if m < ds.num_machines && c.from < ds.machine_seq[m].len() {
            ...
            (1u64 << 60) | ((node & 0xFFFF_FFFF) << 28) | ((c.m_to as u64) & 0x0FFF_FFFF)
        }
        _ => u64::MAX,
    }
}

/// Nowicki-Smutnicki style tabu search over the same critical-block candidates
/// that `descent_phase` explores. Differs from descent_phase in two ways:
///   1. Always picks the best legal move per iteration (improving or not),
///      escaping local optima that descent gets stuck in.
///   2. Maintains a tabu list of recently applied moves keyed by node ids.
///      Aspiration: tabu-listed moves are allowed if they would improve the
///      global best ever seen.
/// Returns the best (DisjSchedule, makespan) found, or None if no improvement
...
fn tabu_phase(
    ...
    top_cands: usize,
    tabu_tenure: usize,
) -> Option<(DisjSchedule, u32)> {
    use std::collections::HashMap;
    let initial_mk = cur_eval.0;
    let mut cur_mk = initial_mk;
    let mut global_best_mk = cur_mk;
    let mut global_best_ds = ds.clone();
    let mut tabu: HashMap<u64, usize> = HashMap::new();
    ...
    for iter in 0..max_iters {
        ...
        // Pick best legal move (excluding tabu unless aspiring).
        let mut best_cand: Option<MoveCand> = None;
        let mut best_mk_after: u32 = u32::MAX;

        for cand in &cands {
            let key = move_tabu_key(ds, cand);
            let is_tabu = tabu.get(&key).map_or(false, |&exp| exp > iter);

            let mk_after = match cand.kind {
                0 => {
                    let m = cand.m_from;
                    if m >= ds.num_machines || cand.from >= ds.machine_seq[m].len() {
                        continue;
                    ...
                    let new_idx = apply_insert(&mut ds.machine_seq[m], cand.from, cand.to);
                    let r = eval_disj(ds, buf).map(|(mk, _)| mk);
                    let _ = apply_insert(&mut ds.machine_seq[m], new_idx, cand.from);
                    r
                }
                2 => {
            ...
            let Some(mk) = mk_after else { continue };
            if is_tabu && mk >= global_best_mk {
                continue;
            }
            if mk < best_mk_after {
                best_mk_after = mk;
        ...
        // Mark this move tabu before applying (key uses pre-move node positions).
        let key = move_tabu_key(ds, &bc);
        tabu.insert(key, iter + tabu_tenure);

        // Apply best move; accept regardless of whether it improves cur_mk.
        let applied_ok = match bc.kind {
            0 => {
                let m = bc.m_from;
                if m < ds.num_machines && bc.from < ds.machine_seq[m].len() {
                    let _ = apply_insert(&mut ds.machine_seq[m], bc.from, bc.to);
                    true
                } else {
                    false
                }
            ...
            1 => apply_reroute(ds, bc.m_from, bc.from, bc.m_to, bc.to, bc.new_pt).is_some(),
            _ => false,
        };
        if !applied_ok {
            break;
        ...
        let Some(new_eval) = eval_disj(ds, buf) else {
            break;
        };
        *cur_eval = new_eval;
        cur_mk = new_eval.0;

        if cur_mk < global_best_mk {
            global_best_mk = cur_mk;
            global_best_ds = ds.clone();
        }
    }

    if global_best_mk < initial_mk {
        Some((global_best_ds, global_best_mk))
    } else {
        None
    }
}

/// Public wrapper: take a Solution, run NS-style tabu search starting from its
/// disjunctive graph, return improved Solution if global best beats input.
pub fn critical_block_tabu_search(
    pre: &Pre,
    challenge: &Challenge,
    base_sol: &Solution,
    tabu_iters: usize,
    ...
    match result {
        Some((ds_best, mk_best)) => {
            // Re-evaluate to populate buf.start for solution reconstruction.
            let mut buf2 = EvalBuf::new(ds_best.n);
            if eval_disj(&ds_best, &mut buf2).is_none() {
                return Ok(None);
            }
            let sol = disj_to_solution(pre, &ds_best, &buf2.start)?;
            Ok(Some((sol, mk_best)))
        }
        None => Ok(None),
    }
