...
pub fn solve_challenge(
    ...
    if density >= 4.22 {
        ...
        solve_low_density_excelled(challenge, save_solution, hyperparameters)
    }
}

fn solve_high_density_suma(
    challenge: &Challenge,
    save_solution: &dyn Fn(&Solution) -> anyhow::Result<()>,
    hyperparameters: &Option<Map<String, Value>>,
) -> anyhow::Result<()> {
                    ...
                    for &l in c.iter() {
                        ...
                        if sad == 0 {
                            ...
                            if min_sad > 0 {
                                ...
                                let age_bonus = (*var_age.get_unchecked(abs_l) as usize) / 2;
                                let combined_weight =
                                    sad * sad * sad_weight + appearances - age_bonus.min(50);
