...
fn score_candidate(
    ...
    match rule {
        ...
        Rule::ShortestProc => {
            ...
            short + rem_min_u * (1.0 + next_u) + sat01_pos(scarcity_urg) + base_bias
                - end_u
                - pop_pen
        }
        Rule::Regret => {
...
fn construct_solution_conflict(
        ...
        |time: u32,
         ...
         job_ready_time: &[u32]| {
            while let Some(std::cmp::Reverse((t, j))) = ready_heap.peek().copied() {
                ...
                if job_next_op[j] >= pre.job_ops_len[j] {
                    continue;
                }
                if job_ready_time[j] != t {
                    continue;
                }
                in_ready[j] = true;
                ready_jobs.push(j);
    ...
    while remaining_ops > 0 {
        ...
        loop {
            let Some(std::cmp::Reverse((t, j))) = ready_heap.peek().copied() else {
                break;
            };
            if t <= time {
                ready_heap.pop();
                if j < num_jobs
                    && !in_ready[j]
                    && job_next_op[j] < pre.job_ops_len[j]
                    && job_ready_time[j] == t
                {
                    in_ready[j] = true;
                    ready_jobs.push(j);
...
#[inline]
fn pick_diverse_top_solution(
    best: &Solution,
    top: &[(Solution, u32)],
    scan: usize,
) -> Option<usize> {
    if top.is_empty() || scan == 0 {
        return None;
    }
...
fn maybe_add_elite(elite: &mut Vec<EliteParams>, cand: EliteParams, cap: usize) {
    ...
    if cand.score < worst {
        if let Some(last) = elite.last_mut() {
            *last = cand;
        } else {
            elite.push(cand);
        }
...
pub fn solve(
    ...
    let horizon = pre
        ...
        .fold(0.0f64, f64::max)
        .max(pre.horizon);
    let time_scale = (horizon
        * (2.65 + 0.15 * pre.load_cv + 0.10 * pre.jobshopness + 0.10 * pre.high_flex))
        .max(1.0);
    ...
    for &rule in &rules {
        let (sol, mk) = construct_solution_conflict(
            challenge, pre, rule, 0, None, &mut rng, None, None, None, 0.0, horizon, time_scale,
        )?;
        commit_best(
            save_solution,
            &mut best_makespan,
    ...
    {
        let jb = job_bias_from_solution(pre, &greedy_sol)?;
        let mp = machine_penalty_from_solution(pre, &greedy_sol, challenge.num_machines)?;
        let rp = route_pref_from_solution_lite(pre, &greedy_sol, challenge)?;
        elite.push(EliteParams {
            jb,
            mp,
            rp,
    ...
    for r in 0..num_restarts {
        ...
        if escape_cooldown == 0 {
            if let Some((sol2, mk2)) = maybe_escape_ls(
                pre,
                challenge,
                &mut rng,
                &top_solutions,
                ...
                if improved {
                    stuck = 0;
                    let jb = job_bias_from_solution(pre, &sol2)?;
                    let mp = machine_penalty_from_solution(pre, &sol2, challenge.num_machines)?;
                    let rp = route_pref_from_solution_lite(pre, &sol2, challenge)?;
                    maybe_add_elite(
                        &mut elite,
                        EliteParams {
                            jb,
        ...
        let k = if k_max <= k_min {
            k_min
        } else if stuck > 120 && rng.gen::<f64>() < 0.55 {
            k_max
        } else {
        ...
        if stuck > 80 && !top_solutions.is_empty() && rng.gen::<f64>() < 0.04 {
            ...
            let jb = job_bias_from_solution(pre, sref)?;
            let mp = machine_penalty_from_solution(pre, sref, challenge.num_machines)?;
            let rp = route_pref_from_solution_lite(pre, sref, challenge)?;
            maybe_add_elite(
                &mut elite,
                EliteParams {
                    jb,
        ...
        let (mut sol, mut mk) = if use_learn {
            ...
            let mp_opt = if rng.gen::<f64>() < drop_mp_p {
                None
            } else {
                Some(&elite[mp_idx].mp)
            };
            let rp_opt = if rng.gen::<f64>() < drop_rp_p {
                None
            } else {
                Some(&elite[rp_idx].rp)
            };
            let jitter = (0.80 + 0.70 * rng.gen::<f64>()).clamp(0.65, 1.55);
            let route_w = if rp_opt.is_some() {
                (route_w_base * jitter).clamp(route_w_base * 0.55, 0.45)
            } else {
                0.0
            };
            ...
            construct_solution_conflict(
                challenge, pre, rule, k, target, &mut rng, None, None, None, 0.0, horizon,
                time_scale,
            )?
        };

        if let Some((sol2, mk2)) = maybe_intensify_ls(
            pre,
            challenge,
            &mut rng,
            &sol,
    ...
    for (base_sol, _) in top_solutions.iter() {
        ...
        for attempt in 0..10 {
            ...
            .min(k_hi);
            let mut mp_ref: Option<&Vec<f64>> = Some(&mp_base);
            let mut rp_ref: Option<&RoutePrefLite> = Some(&rp_base);
            if !elite.is_empty() && rng.gen::<f64>() < mix_ref_p {
                let eidx = pick_elite_idx(&mut rng, &elite);
                if rng.gen::<f64>() < 0.62 {
                    mp_ref = Some(&elite[eidx].mp);
                }
                if rng.gen::<f64>() < 0.72 {
                    rp_ref = Some(&elite[eidx].rp);
                }
                if rng.gen::<f64>() < 0.055 {
                    rp_ref = None;
                }
            }
            let rw_j = if rp_ref.is_some() {
                (route_w_ls * (0.86 + 0.50 * rng.gen::<f64>())).clamp(route_w_ls * 0.70, 0.45)
            } else {
                0.0
            };
            let (mut sol, mut mk) = construct_solution_conflict(
                challenge,
                pre,
                rule,
                k,
            ...
            if commit_best(
                ...
                let jb2 = job_bias_from_solution(pre, &sol)?;
                let mp2 = machine_penalty_from_solution(pre, &sol, challenge.num_machines)?;
                let rp2 = route_pref_from_solution_lite(pre, &sol, challenge)?;
                maybe_add_elite(
                    &mut elite,
                    EliteParams {
                        jb: jb2,
    ...
    for i in 0..ls_runs {
        ...
        if let Some((sol2, mk2)) =
            critical_block_move_local_search_ex(pre, challenge, base_sol, 40, 64, 12)?
        {
            commit_best(
                save_solution,
                &mut best_makespan,
    ...
    if let Some(ref sol) = best_solution.clone() {
        if pre.high_flex + pre.jobshopness > 0.55 {
            if let Some((sol2, mk2)) =
                critical_block_move_local_search_ex(pre, challenge, sol, 50, 80, 14)?
            {
                commit_best(
                    save_solution,
                    &mut best_makespan,
