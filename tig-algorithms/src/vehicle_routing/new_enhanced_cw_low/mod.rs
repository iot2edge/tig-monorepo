...
mod dead_code {
    ...
    pub fn solve_sub_instance(challenge: &SubInstance) -> anyhow::Result<Option<SubSolution>> {
        ...
        for outer_iter in 0..OUTER_ITERATIONS {
            ...
            for inner_iter in 0..inner_iterations {
                ...
                if (current_cost as f32 * 0.95) > challenge.baseline_total_distance as f32
                    && !promising
                {
                    return Ok(None);
                } else {
                ...
                let mut best_solution = Some(SubSolution {
                ...
                iteration_results.push((current_params, best_cost));
            ...
            if outer_iter < OUTER_ITERATIONS - 1 {
                let (best_params, best_cost) = iteration_results
                    .iter()
    ...
    impl Savings {
        ...
        pub fn build_supplementary_structs(&mut self, challenge: &SubInstance) {
            ...
            for &(_, i16, j16) in &self.stable_list {
                ...
                let saving = challenge.distance_matrix[0][i] + challenge.distance_matrix[j][0]
                ...
                for &i in modified_indices {
                    for k in 0..mask_len {
                        ...
                        while unvisited_pairs_mask != 0 {
                            ...
                            let new_score =
                                (*params.get_unchecked(i) + *params.get_unchecked(j)) * base_saving;
    ...
    #[inline(never)]
    fn create_solution(challenge: &SubInstance, savings_list: &[(u32, u16, u16)]) -> SubSolution {
        let num_nodes = challenge.num_nodes;
        ...
        let mut route: Vec<(usize, usize)> = (0..num_nodes).map(|i| (i, i)).collect();
        ...
        for &(_, i16, j16) in savings_list {
            ...
            if route_i_start == route_j_start
                || route_i_start == route_j_end
                || route_i_end == route_j_start
                || route_i_end == route_j_end
            {
                continue;
            }
        ...
        let final_routes =
            extract_routes(num_nodes, &route_demands, max_capacity, &route, &node_links);
