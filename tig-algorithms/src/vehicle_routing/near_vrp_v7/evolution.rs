...
impl<'a> Evolution<'a> {
    ...
    // ILS-style diversification: ruin-and-recreate the incumbent best, then
    // re-optimise with the existing local search and re-admit. Fired only when
    // the genetic search has stagnated (see run()).
    fn sisr_kick(&mut self, rng: &mut SmallRng, ls: &mut LocalOps, sisr: &SisrOps) {
        self.last_feasible_cost = i64::MAX;

        let base = match self.population.best_feasible() {
            Some(b) => b,
            None => {
                self.generate_crossover_individual(rng, ls);
                return;
            }
        };
        ...
        sisr.recreate_greedy(&mut routes, removed, rng, self.params.sisr_blink);
        self.educate_split_child(&mut routes, rng, ls);

        let child = Individual::new_from_routes(self.data, &self.params, routes);
        let is_capa_feasible = child.load_excess == 0;
        let is_tw_feasible = child.tw_violation == 0;
        if is_capa_feasible && is_tw_feasible && child.cost < self.last_feasible_cost {
            self.last_feasible_cost = child.cost;
        }
    ...
    pub fn run(
        ...
        let sisr = if self.params.sisr_enabled && self.params.sisr_kick_interval > 0 {
            Some(SisrOps::new(self.data))
        } else {
            None
        };
        while it_noimprov < self.params.max_it_noimprov && it_total < self.params.max_it_total {
            let do_kick = sisr.is_some()
                && it_noimprov > 0
                && it_noimprov % self.params.sisr_kick_interval == 0;
            if do_kick {
                let s = sisr.as_ref().unwrap();
                self.sisr_kick(rng, &mut ls, s);
            } else {
