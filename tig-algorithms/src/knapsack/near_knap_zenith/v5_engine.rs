// V5 ENGINE: kqo_v5 source verbatim, isolated as a sub-module to avoid name
// conflicts with v6's identically-named items (Solver, Hparams, etc.).
// Used as a third pass in zenith's solve_challenge — wins n=1000,b=25 where
// v6 source can't (structural difference, not just HP).
...
impl Solver {
    pub fn solve(
        ...
        let n_phase1 = n_restarts.min(10).max(2);
