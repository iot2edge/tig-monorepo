...
impl<'a> Evolution<'a> {
    ...
    pub fn run(
        ...
        // Restart loop: run HGS up to 3 times, take the best across restarts
        let n_restarts: usize = 3;
        for restart_idx in 0..n_restarts {
            if restart_idx > 0 {
                self.population = GenePool::new(self.data);
            }

            let diversity_boost = if self.data.nb_nodes < 1000 { 3 } else { 1 };
            for it in 0..(self.params.mu_start + diversity_boost) {
                self.generate_initial_individual(rng, &mut ls, it > 0 || restart_idx > 0);
            }
            ...
            while it_noimprov < self.params.max_it_noimprov && it_total < self.params.max_it_total {
                ...
                if cur.better_than(best_metric) {
                    ...
                    if let Some(best) = self.population.best_feasible() {
                        let is_new_global = match &global_best {
                            None => true,
                            Some(g) => best.distance < g.distance,
                        };
                        if is_new_global {
                            if let Some(save) = save_solution {
                                let _ = save(&Solution {
