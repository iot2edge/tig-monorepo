...
fn score_candidate(
    ...
    let route_term = if route_w > 0.0 && op.flex >= 2 {
        route_w * route_gain * route_pref_bonus_fh(route_pref, product, op_idx, machine)
    } else {
        0.0
    };
    match rule {
...
fn cp_window_exhaustive(
    ...
    for _iter in 0..max_iters {
        ...
        for &nd in &order {
            ...
            if let Some(pos) = seq.iter().position(|&x| x == nd) {
                if pos > 0 {
                    ...
                    if finish[pred_nd] == buf.start[nd] {
                        on_cp[pred_nd] = true;
                    }
                }
            }
        ...
        for &nd in &cp_flex {
            machine_cp_count[ds.node_machine[nd]] += 1;
        }
        let bottleneck_m = machine_cp_count
            .iter()
            .enumerate()
            .max_by_key(|&(_, &c)| c)
            .map(|(m, _)| m)
        ...
        'window_search: for &wsize in &window_sizes {
            if cp_flex.len() < wsize {
                continue;
            }
            let bottleneck_ops: Vec<usize> = cp_flex
                .iter()
                .copied()
                .filter(|&nd| ds.node_machine[nd] == bottleneck_m)
                .collect();
            if bottleneck_ops.is_empty() {
                continue;
            }
            ...
            for w_start in 0..num_windows {
                ...
                let orig: Vec<(usize, u32, usize)> = window
                    .iter()
                    .map(|&nd| {
                        let cur_m = ds.node_machine[nd];
                        let cur_pt = ds.node_pt[nd];
                        let cur_pos = ds.machine_seq[cur_m]
                            .iter()
                            .position(|&x| x == nd)
                            .unwrap_or(0);
                ...
                let options: Vec<Vec<(usize, u32)>> = window
                    .iter()
                    .map(|&nd| {
                        let job = ds.node_job[nd];
                        let op_idx = ds.node_op[nd];
                        let product = pre.job_products[job];
                        if op_idx < pre.product_ops[product].len() {
                            pre.product_ops[product][op_idx]
                                .machines
                                .iter()
                                .copied()
                                .collect()
                    ...
                    if apply_ok {
                        let affected_machines: Vec<usize> = {
                            let mut ms: Vec<usize> =
                                window.iter().map(|&nd| ds.node_machine[nd]).collect();
                        ...
                        if let Some((tmk, _)) = eval_disj(&ds, &mut buf) {
                            if tmk < best_combo_mk {
                                ...
                                best_combo = Some(
                                    window
                                        .iter()
                                        .map(|&nd| (ds.node_machine[nd], ds.node_pt[nd]))
                                        .collect(),
                                );
                            }
                        }
                ...
                if let Some(best_assign) = best_combo {
                    if best_combo_mk < current_mk {
                        for (wi, &nd) in window.iter().enumerate() {
                            ...
                            if new_m != cur_m {
                                let old_pos =
                                    ds.machine_seq[cur_m].iter().position(|&x| x == nd).unwrap();
                                ds.machine_seq[cur_m].remove(old_pos);
                                ds.node_machine[nd] = new_m;
                                ds.node_pt[nd] = new_pt;
                                ds.machine_seq[new_m].push(nd);
...
pub fn solve(
    ...
    for r in 0..num_restarts {
        ...
        if mk < best_makespan {
            ...
            if learn_updates_left > 0 {
                let (jb, mp, rp) = consensus_learning_from_elites(pre, challenge, &top_solutions)?;
                learned_jb = Some(jb);
                learned_mp = Some(mp);
                learned_rp = Some(rp);
    ...
    for i in 0..ls_runs {
        ...
        if let Some((sol2, mk2)) =
            critical_block_move_local_search_ex(pre, challenge, base_sol, 8, 128, 24)?
        {
