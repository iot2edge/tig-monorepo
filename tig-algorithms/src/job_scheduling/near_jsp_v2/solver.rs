...
pub fn solve_challenge(
    ...
    let optimizer_result: Result<()> = match detected {
        ...
        DetectedTrack::Parallel => {
            // ≈ HYBRID_FLOW_SHOP — v4's hybrid_flow_shop scores 41,758.
            let pre = v4_build_pre(challenge)?;
            let effort = parse_v4_effort(hyperparameters);
        ...
        DetectedTrack::Chaotic => {
            // ≈ FJSP_HIGH — fuel-adaptive: same reasoning as fjsp_medium.
            if fuel_remaining() < HIGH_FUEL_THRESHOLD {
                let pre = v4_build_pre(challenge)?;
                let effort = parse_v4_effort(hyperparameters);
                fjsp_high::solve(challenge, save, &pre, &effort)
            } else {
                let pre = v3_build_pre(challenge)?;
                let effort = V3Effort::default_effort();
                fs_fjsp_high::solve(challenge, save, &pre, &effort)
            }
        }
    };
