...
impl PD {
    ...
    #[inline]
    fn op_id(&self, j: usize, k: usize) -> usize {
        self.job_offset[j] + k
    }
}
...
impl DG {
    fn from_solution(pd: &PD, c: &Challenge, sol: &Solution) -> Result<Self> {
        ...
        for j in 0..nj {
            ...
            for k in 0..pd.job_ops_len[j] {
                ...
                let pt = c.product_processing_times[p][k]
                    .get(&m)
                    .copied()
                    .unwrap_or(1);
                node_pt[o] = pt;
        ...
        Ok(DG {
            ...
            node_job,
            node_op,
            job_offset: pd.job_offset.clone(),
            job_ops_len: pd.job_ops_len.clone(),
            machine_seq,
        })
    }
    ...
    fn compute_tails_and_preds(&self, buf: &mut Buf) {
        ...
        for seq in &self.machine_seq {
            for i in 1..seq.len() {
                buf.mpred[seq[i]] = seq[i - 1];
            }
        }
        for j in 0..self.nj {
            let base = self.job_offset[j];
            for k in 1..self.job_ops_len[j] {
                buf.jpred[base + k] = base + k - 1;
            }
        }
        ...
        for &o in buf.stack.iter().rev() {
            ...
            if k > 0 {
                ...
                if contrib > buf.tail[jp] {
                    buf.tail[jp] = contrib;
                }
            }
            if buf.mpred[o] != NONE {
                let mp = buf.mpred[o];
                if contrib > buf.tail[mp] {
                    buf.tail[mp] = contrib;
                }
            }
        }
...
fn estimate_insert_mk(
    ...
    let js = if k + 1 < dg.job_ops_len[j] {
        dg.job_offset[j] + k + 1
    } else {
        NONE
    };
    let r_from_job = if jp != NONE {
        buf.start[jp] + dg.node_pt[jp]
    } else {
        0
    };
    let q_from_job = if js != NONE {
        dg.node_pt[js] + buf.tail[js]
    } else {
        0
    };
    ...
    let r_mach = if mp != NONE {
        buf.start[mp] + dg.node_pt[mp]
    } else {
        0
    };
    let new_r_op = r_from_job.max(r_mach);
    let q_mach = if ms != NONE {
        dg.node_pt[ms] + buf.tail[ms]
    } else {
        0
    };
    ...
    let path_gap = if old_ms != NONE {
        ...
        let jpp = if kk > 0 {
            dg.job_offset[jj] + kk - 1
        } else {
            NONE
        };
        let r_j = if jpp != NONE {
            buf.start[jpp] + dg.node_pt[jpp]
        } else {
            0
        };
        let r_m = if old_mp != NONE {
            buf.start[old_mp] + dg.node_pt[old_mp]
        } else {
            0
        };
    ...
    for m in 0..dg.nm {
        ...
        while i < seq.len() {
            ...
            while be + 1 < seq.len() {
                ...
                if buf.start[y] != buf.start[x] + dg.node_pt[x] {
                    break;
                }
                be += 1;
            }
            if be > bs {
                blocks.push(CB { m, s: bs, e: be });
            }
            i = be + 1;
        }
    }
...
fn estimate_swap_mk(m: usize, pos: usize, buf: &Buf, dg: &DG) -> u32 {
    ...
    let ms = if pos + 2 < seq.len() {
        seq[pos + 2]
    } else {
        NONE
    };

    let end_mp = if mp != NONE {
        buf.start[mp] + dg.node_pt[mp]
    } else {
        0
    };

    let j2 = dg.node_job[o2];
    let k2 = dg.node_op[o2];
    let jp2 = if k2 > 0 {
        dg.job_offset[j2] + k2 - 1
    } else {
        NONE
    };
    let end_jp2 = if jp2 != NONE {
        buf.start[jp2] + dg.node_pt[jp2]
    } else {
        0
    };
    let r_o2 = end_mp.max(end_jp2);

    let j1 = dg.node_job[o1];
    let k1 = dg.node_op[o1];
    let jp1 = if k1 > 0 {
        dg.job_offset[j1] + k1 - 1
    } else {
        NONE
    };
    let end_jp1 = if jp1 != NONE {
        buf.start[jp1] + dg.node_pt[jp1]
    } else {
        0
    };
    let r_o1 = (r_o2 + dg.node_pt[o2]).max(end_jp1);

    let js1 = if k1 + 1 < dg.job_ops_len[j1] {
        dg.job_offset[j1] + k1 + 1
    } else {
        NONE
    };
    let q_j1 = if js1 != NONE {
        dg.node_pt[js1] + buf.tail[js1]
    } else {
        0
    };
    let q_ms = if ms != NONE {
    ...
    let js2 = if k2 + 1 < dg.job_ops_len[j2] {
        dg.job_offset[j2] + k2 + 1
    } else {
        NONE
    };
    let q_j2 = if js2 != NONE {
        dg.node_pt[js2] + buf.tail[js2]
    } else {
        0
    };
    ...
    let path3 = if ms != NONE {
        let jms = dg.node_job[ms];
        let kms = dg.node_op[ms];
        let jpms = if kms > 0 {
            dg.job_offset[jms] + kms - 1
        } else {
            NONE
        };
        let end_jpms = if jpms != NONE {
            buf.start[jpms] + dg.node_pt[jpms]
        } else {
            0
        };
...
fn hybrid_search(
    ...
    let Some((mk, mk_node)) = dg.eval(buf) else {
        return Ok((u32::MAX, dg.clone()));
    };
    ...
    let tenure = if high_flex {
        4 + (pd.n as f64).sqrt() as usize / 2
    } else {
        7 + (pd.n as f64).sqrt() as usize
    };
    ...
    for iter in 0..max_iter {
        ...
        for block in &blocks {
            ...
            for target in (bs + 1)..=be {
                let est =
                    estimate_insert_mk(first_op, m, bs, m, target, dg.node_pt[first_op], buf, dg);
                cands.push(Cand {
                    kind: 1,
                    op: first_op,
                    m,
            ...
            for target in bs..be {
                let est =
                    estimate_insert_mk(last_op, m, be, m, target, dg.node_pt[last_op], buf, dg);
                cands.push(Cand {
                    kind: 1,
                    op: last_op,
                    m,
        ...
        if high_flex {
            for o in 0..pd.n {
                if !buf.saved_crit[o] || pd.op_flex[o] < 2 {
                    continue;
                }
...
fn perturb(dg: &mut DG, pd: &PD, buf: &mut Buf, strength: usize, pseed: &mut u64) {
    let Some((mk, mkn)) = dg.eval(buf) else {
        return;
    };
    dg.compute_tails_and_preds(buf);
    dg.mark_critical(buf, mk, mkn);
    ...
    for _ in 0..strength {
        ...
        *pseed ^= (*pseed).wrapping_shl(17);
        ...
        if !crit_flex.is_empty() && (*pseed % 3) == 0 {
            ...
            *pseed ^= (*pseed).wrapping_shl(17);
            ...
            if new_m != old_m {
                if let Some(from_pos) = dg.machine_seq[old_m].iter().position(|&x| x == node) {
                    ...
                    let near = dg.machine_seq[new_m].partition_point(|&x| buf.start[x] < jpred_end);
            ...
            // v10 fix: was `if blen < 1`. The swap operates on (pos, pos+1)
            // so we need at least 2 adjacent slots inside the block. The
            // original guard let blen==1 through, computed pos=block.s and
            // swapped (block.s, block.s+1) — out of bounds whenever block.s
            // is the last element of machine_seq. Same root cause as the
            // job_tree/job_four/job_five panics observed in round-117 bench.
            if blen < 2 {
                continue;
            }
            *pseed ^= (*pseed).wrapping_shl(13);
            *pseed ^= (*pseed).wrapping_shr(7);
            *pseed ^= (*pseed).wrapping_shl(17);
            // v10 fix: original `% blen` allowed pos = block.e - 1, giving
            // pos+1 = block.e — past the end of the block (and possibly
            // past machine_seq.len() when the block reaches the tail).
            // `% (blen - 1)` caps pos at block.e - 2 so pos+1 = block.e - 1
            // stays in-block.
            let pos = block.s + (*pseed as usize) % (blen - 1);
            // v15 fix: even with the v10 modular fix, blocks computed once
            // at the top of perturb() can be invalidated by the
            // reassign-machine branch above (which removes a node from
            // machine_seq[old_m] and inserts into machine_seq[new_m]).
            // When that happens before this swap branch fires, block.e may
            // exceed machine_seq[block.m].len() and the swap panics with
            // "index out of bounds" (observed 1/20 nonces on fjsp_medium
            // in the v14 20-nonce bench). Cheap runtime bounds check fixes
            // it without recomputing blocks every iteration.
            if pos + 1 < dg.machine_seq[block.m].len() {
...
fn construct_mc(pd: &PD, rule: usize, rng: &mut SmallRng, top_k: usize) -> (Solution, u32) {
    ...
    let mut schedule: Vec<Vec<(usize, u32)>> = pd
        .job_ops_len
        .iter()
    ...
    let mut job_rem_work: Vec<f64> = (0..nj)
        .map(|j| {
            (0..pd.job_ops_len[j])
                .map(|k| {
                    let o = pd.op_id(j, k);
    ...
    while remaining > 0 {
        ...
        for &m in &m_order {
            ...
            for j in 0..nj {
                ...
                for &(em, ept) in &pd.op_elig[o] {
                    ...
                    if eend < earliest_end {
                        earliest_end = eend;
                    }
                }
                if end_on_m != earliest_end {
                    continue;
                }
        ...
        for mm in 0..nm {
            if m_avail[mm] > time && m_avail[mm] < next_time {
                next_time = m_avail[mm];
            }
        }
        for j in 0..nj {
...
fn construct(pd: &PD, rule: usize, rng: &mut SmallRng, top_k: usize) -> (Solution, u32) {
    ...
    let mut job_rem_work: Vec<f64> = (0..nj)
        .map(|j| {
            (0..pd.job_ops_len[j])
                .map(|k| pd.op_min_pt[pd.op_id(j, k)] as f64)
                .sum()
        })
        .collect();
    ...
    while remaining > 0 {
        ...
        let pick = if top_k > 1 && cands.len() > 1 {
            rng.gen_range(0..top_k.min(cands.len()))
        } else {
            0
        };
...
pub fn solve_our(
    ...
    let (num_restarts, keep_top, search_iters, final_iters, perturb_str, noi_limit, verify_k) =
        if pd.flex_avg > 5.0 {
            ...
            // FjspMedium — v13 cranks doubled iters from v4's "reduced for
            // speed" defaults (2000, 6, 5000, 12000, 4, 80, 20). At 5T cap,
            // v12 ensemble + solve_our default params only reaches 74,135;
            // job_nine.so produces 81,361 on the same instances. Hypothesis:
            // job_nine.so calls solve_our with higher iteration counts (the
            // "still good quality" comment implies these were trimmed from
            // a stronger production tuning).
            (4000, 10, 10000, 24000, 4, 120, 24)
        } else if high_flex {
    ...
    let mut pseed: u64 = (challenge.seed[0] as u64).wrapping_mul(0x9E3779B97F4A7C15)
        ^ (best_mk as u64).wrapping_shl(16)
        ^ (pd.n as u64);
