...
// job_eight-derived (`je_` prefix) modules: upgrade the fjsp_high path. On
// chain (round 117) job_eight scores ~49,217 on fjsp_high vs v4's 47,797.
...
pub fn solve_challenge(
    ...
    let optimizer_result: Result<()> = match detected {
        DetectedTrack::Random => {
            // ≈ JOB_SHOP — v1's track_random scores 26,105 (best on the field).
        ...
        DetectedTrack::Strict => {
            // ≈ FLOW_SHOP — adaptive strategy based on starting fuel budget:
            //   * High fuel (>= 5B): skip verifier_baseline. The dual-engine
            //     (track_strict + fs_flow_shop) reliably beats verifier's
            //     greedy on its own. Saves the baseline's fuel cost (~12k
            //     quality recovered).
            //   * Low fuel (< 5B): run verifier_baseline first as a floor,
            //     then just fs_flow_shop with the remaining fuel. Guarantees
            //     0 invalid even when the dual-engine would otherwise fall
            //     short of beating verifier's greedy.
            if fuel_remaining() < LOW_FUEL_THRESHOLD {
                let _ = verifier_baseline::solve_challenge_with_effort(challenge, save, 0);

                let pre3 = v3_build_pre(challenge)?;
                let effort3 = V3Effort::default_effort();
                let _ = fs_flow_shop::solve(challenge, save, &pre3, &effort3);
            } else {
                let (greedy_sol, greedy_mk) = run_simple_greedy_baseline(challenge)?;
                save(&greedy_sol)?;
                let pre1 = v1_build_pre(challenge)?;
                let effort1 = parse_v1_effort(hyperparameters);
                js_track_strict::solve(challenge, save, &pre1, greedy_sol, greedy_mk, &effort1)?;
        ...
        DetectedTrack::Parallel => {
            // ≈ HYBRID_FLOW_SHOP — v4's hybrid_flow_shop scores 41,758 at default
            // iters=2000. Apply same Phase-1-reduction pattern: cut iters so
            // the refinement phases get more fuel.
            let pre = v4_build_pre(challenge)?;
            let mut effort = parse_v4_effort(hyperparameters);
            effort = effort.with_hybrid_flow_shop_iters(500);
            hybrid_flow_shop::solve(challenge, save, &pre, &effort)
        }
        DetectedTrack::Complex => {
            // ≈ FJSP_MEDIUM — fuel-adaptive:
            //   * Low fuel (< 30B): v4's fjsp_medium (faster, higher quality).
            //   * High fuel (≥ 30B): v3's fjsp_medium (v4's becomes invalid
            //     here, producing solutions worse than verifier's greedy).
            if fuel_remaining() < HIGH_FUEL_THRESHOLD {
                let pre = v4_build_pre(challenge)?;
                // Tuning: reduce fjsp_medium_iters from default 2000 to 500.
                // Same pattern as je_fjsp_high — Phase 1 over-spends fuel that
                // Phase 2-8 refinement uses more productively.
                let mut effort = parse_v4_effort(hyperparameters);
                effort = effort.with_fjsp_medium_iters(500);
                fjsp_medium::solve(challenge, save, &pre, &effort)
            } else {
                let pre = v3_build_pre(challenge)?;
                let effort = V3Effort::default_effort();
                fs_fjsp_medium::solve(challenge, save, &pre, &effort)
            }
        }
