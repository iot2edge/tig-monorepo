...
fn score_candidate(
    ...
    let route_term = if route_w > 0.0 && op.flex >= 2 {
        route_w * route_gain * route_pref_bonus_hfs(route_pref, product, op_idx, machine)
    } else {
        0.0
    };
    match rule {
...
fn maybe_escape_ls(
    ...
    let idx = pick_top_idx(rng, top_solutions);
    ...
    for r in 0..num_restarts {
        ...
        if escape_cooldown == 0 {
            if let Some((sol2, mk2)) =
                maybe_escape_ls(pre, challenge, &mut rng, &top_solutions, stuck, flex01)?
            {
                escape_cooldown = 14;
